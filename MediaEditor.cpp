...
static void ShowMediaPreviewWindow(ImDrawList *draw_list, std::string title, ImRect& video_rect, bool audio_bar = true, bool monitors = true, bool force_update = false)
{
    ...
    if (!frame.empty() && 
        (timeline->mLastFrameTime == -1 || timeline->mLastFrameTime != (float)(frame.time_stamp * 1000) || need_update_scope || force_update))
    {
...
static void ShowVideoFilterWindow(ImDrawList *draw_list)
{
    ...
    if (ImGui::BeginChild("##video_filter_preview", video_preview_size, false, child_flags))
    {
        ...
        if (timeline->mVidFilterClip)
        {
            ...
            if (ret && 
                (timeline->mVidFilterClip->mLastFrameTime == -1 || timeline->mVidFilterClip->mLastFrameTime != (float)(pair.first.time_stamp * 1000) || need_update_scope))
            {
...
static void ShowVideoFusionWindow(ImDrawList *draw_list)
{
    ...
    if (ImGui::BeginChild("##video_fusion_preview", video_preview_size, false, child_flags))
    {
        ...
        if (timeline->mVidOverlap)
        {
            ...
            if (ret && 
                (timeline->mVidOverlap->mLastFrameTime == -1 || timeline->mVidOverlap->mLastFrameTime != (float)(pair.first.first.time_stamp * 1000) || need_update_scope))
            {
                CalculateVideoScope(pair.second);
