...
#include "WindowSlice_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
WindowSlice_vulkan::WindowSlice_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "WindowSlice");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(WindowSlice_data, opt, spirv_data) == 0)
    {
...
WindowSlice_vulkan::~WindowSlice_vulkan()
{
    if (vkdev)
    {
...
void WindowSlice_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float smoothness, float count) const
{
    ...
    constants[17].f = count;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double WindowSlice_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float smoothness, float count) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, smoothness, count);
