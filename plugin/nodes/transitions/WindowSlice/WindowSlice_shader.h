...
{ \n\
    float pr = smoothstep(-p.smoothness, 0.0, point.x - p.progress * (1.0 + p.smoothness)); \n\
    float s = step(pr, fract(p.count * point.x)); \n\
    ...
    return mix(rgba_from, rgba_to, sfp(s)); \n\
} \n\
\n\
void main() \n\
{ \n\
...
static const char WindowSlice_data[] = 
SHADER_HEADER
SHADER_PARAM
