...
#include "CannabisLeaf_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
CannabisLeaf_vulkan::CannabisLeaf_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "CannabisLeaf");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(CannabisLeaf_data, opt, spirv_data) == 0)
    {
...
CannabisLeaf_vulkan::~CannabisLeaf_vulkan()
{
    if (vkdev)
    {
...
void CannabisLeaf_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress) const
{
...
double CannabisLeaf_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
