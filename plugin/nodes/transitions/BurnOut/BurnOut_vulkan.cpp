...
#include "BurnOut_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
BurnOut_vulkan::BurnOut_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "BurnOut");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(BurnOut_data, opt, spirv_data) == 0)
    {
...
void BurnOut_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, ImPixel& shadow_colour, float smoothness) const
{
    ...
    constants[17].f = shadow_colour.r;
    constants[18].f = shadow_colour.g;
    constants[19].f = shadow_colour.b;
    constants[20].f = shadow_colour.a;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double BurnOut_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, ImPixel& shadow_colour, float smoothness) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, shadow_colour, smoothness);
