...
#include "Swirl_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Swirl_vulkan::Swirl_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Swirl");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Swirl_data, opt, spirv_data) == 0)
    {
...
void Swirl_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float radius) const
{
    ...
    constants[16].f = radius;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Swirl_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float radius) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, radius);
