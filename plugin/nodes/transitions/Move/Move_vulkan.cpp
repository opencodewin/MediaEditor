...
#include "Move_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Move_vulkan::Move_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Move");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Move_data, opt, spirv_data) == 0)
    {
...
void Move_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float direction_x, float direction_y) const
{
...
double Move_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float direction_x, float direction_y) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, direction_x, direction_y);
