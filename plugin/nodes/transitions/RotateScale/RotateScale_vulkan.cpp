...
#include "RotateScale_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
RotateScale_vulkan::RotateScale_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "RotateScale");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(RotateScale_data, opt, spirv_data) == 0)
    {
...
RotateScale_vulkan::~RotateScale_vulkan()
{
    if (vkdev)
    {
...
void RotateScale_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, ImPixel& back_colour, float rotations, float scale) const
{
    ...
    std::vector<vk_constant_type> constants(22);
    ...
    constants[16].f = rotations;
    constants[17].f = scale;
    constants[18].f = back_colour.r;
    constants[19].f = back_colour.g;
    constants[20].f = back_colour.b;
    constants[21].f = back_colour.a;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double RotateScale_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, ImPixel& back_colour, float rotations, float scale) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, back_colour, rotations, scale);
