...
#include "Wipe_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Wipe_vulkan::Wipe_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Wipe");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Wipe_data, opt, spirv_data) == 0)
    {
...
void Wipe_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, int direction) const
{
...
double Wipe_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, int direction) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
