...
#include "Dissolve_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Dissolve_vulkan::Dissolve_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Dissolve");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Dissolve_data, opt, spirv_data) == 0)
    {
...
Dissolve_vulkan::~Dissolve_vulkan()
{
    if (vkdev)
    {
...
void Dissolve_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, ImPixel spread_colour, ImPixel hot_colour, float line_width, float pow, float intensity) const
{
    ...
    std::vector<vk_constant_type> constants(27);
    ...
    constants[16].f = line_width;
    constants[17].f = pow;
    constants[18].f = intensity;
    constants[19].f = spread_colour.r;
    constants[20].f = spread_colour.g;
    constants[21].f = spread_colour.b;
    constants[22].f = spread_colour.a;
    constants[23].f = hot_colour.r;
    constants[24].f = hot_colour.g;
    constants[25].f = hot_colour.b;
    constants[26].f = hot_colour.a;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Dissolve_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, ImPixel spread_colour, ImPixel hot_colour, float line_width, float pow, float intensity) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, spread_colour, hot_colour, line_width, pow, intensity);
