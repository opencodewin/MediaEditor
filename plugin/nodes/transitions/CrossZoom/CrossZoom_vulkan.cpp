...
#include "CrossZoom_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
CrossZoom_vulkan::CrossZoom_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "CrossZoom");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(CrossZoom_data, opt, spirv_data) == 0)
    {
...
CrossZoom_vulkan::~CrossZoom_vulkan()
{
    if (vkdev)
    {
...
void CrossZoom_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float strength) const
{
    ...
    constants[16].f = strength;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double CrossZoom_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float strength) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, strength);
