...
#include "SquaresWire_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
SquaresWire_vulkan::SquaresWire_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "SquaresWire");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(SquaresWire_data, opt, spirv_data) == 0)
    {
...
SquaresWire_vulkan::~SquaresWire_vulkan()
{
    if (vkdev)
    {
...
void SquaresWire_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float smoothness, int size, float direction_x, float direction_y) const
{
    ...
    constants[17].i = size;
    constants[18].f = direction_x;
    constants[19].f = direction_y;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double SquaresWire_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float smoothness, int size, float direction_x, float direction_y) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, smoothness, size, direction_x, direction_y);
