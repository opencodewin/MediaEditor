...
#include "Rolls_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Rolls_vulkan::Rolls_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Rolls");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Rolls_data, opt, spirv_data) == 0)
    {
...
void Rolls_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, bool RotDown, int type) const
{
    ...
    constants[16].i = RotDown ? 1 : 0;
    constants[17].i = type;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Rolls_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, bool RotDown, int type) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, RotDown, type);
