...
{ \n\
    float dist = length((vec2(point) - 0.5) * ratio2); \n\
    ...
    return mix( \n\
        p.progress < 0.5 ? rgba_from : rgba_to, // branching is ok here as we statically depend on progress uniform (branching won't change over pixels) \n\
        bgcolor, \n\
        sfp(step(s, dist)) \n\
    ); \n\
} \n\
...
static const char CircleCrop_data[] = 
SHADER_HEADER
SHADER_PARAM
