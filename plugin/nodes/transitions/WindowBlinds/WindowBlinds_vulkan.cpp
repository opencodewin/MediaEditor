...
#include "WindowBlinds_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
WindowBlinds_vulkan::WindowBlinds_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "WindowBlinds");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(WindowBlinds_data, opt, spirv_data) == 0)
    {
...
WindowBlinds_vulkan::~WindowBlinds_vulkan()
{
    if (vkdev)
    {
...
void WindowBlinds_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress) const
{
...
double WindowBlinds_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
