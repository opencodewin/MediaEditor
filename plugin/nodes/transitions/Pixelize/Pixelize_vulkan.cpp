...
#include "Pixelize_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Pixelize_vulkan::Pixelize_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Pixelize");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Pixelize_data, opt, spirv_data) == 0)
    {
...
Pixelize_vulkan::~Pixelize_vulkan()
{
    if (vkdev)
    {
...
void Pixelize_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, int size, int steps) const
{
...
double Pixelize_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, int size, int steps) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, size, steps);
