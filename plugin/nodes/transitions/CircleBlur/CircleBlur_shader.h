...
{ \n\
    float x = p.open == 1 ? p.progress : 1. - p.progress; \n\
    float m = smoothstep(-p.smoothness, 0.0, SQRT_2 * distance(center, point) - x * (1. + p.smoothness)); \n\
    ...
    return mix(rgba_from, rgba_to, p.open == 1 ? sfp(1. - m) : sfp(m)); \n\
} \n\
\n\
void main() \n\
{ \n\
...
static const char CircleBlur_data[] = 
SHADER_HEADER
SHADER_PARAM
