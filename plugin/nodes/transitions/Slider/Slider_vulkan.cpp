...
#include "Slider_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Slider_vulkan::Slider_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Slider");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Slider_data, opt, spirv_data) == 0)
    {
...
void Slider_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, bool out, int type) const
{
    ...
    constants[16].i = out ? 0 : 1; // in
    constants[17].i = type;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Slider_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, bool out, int type) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, out, type);
