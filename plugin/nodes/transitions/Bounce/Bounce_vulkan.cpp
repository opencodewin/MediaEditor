...
#include "Bounce_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Bounce_vulkan::Bounce_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Bounce");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Bounce_data, opt, spirv_data) == 0)
    {
...
void Bounce_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, ImPixel& shadow_colour, float shadow_height, float bounces) const
{
    ...
    constants[16].f = shadow_height;
    constants[17].f = bounces;
    constants[18].f = shadow_colour.r;
    constants[19].f = shadow_colour.g;
    constants[20].f = shadow_colour.b;
    constants[21].f = shadow_colour.a;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Bounce_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, ImPixel& shadow_colour, float shadow_height, float bounces) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, shadow_colour, shadow_height, bounces);
