...
#include "Crosshatch_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Crosshatch_vulkan::Crosshatch_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Crosshatch");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Crosshatch_data, opt, spirv_data) == 0)
    {
...
Crosshatch_vulkan::~Crosshatch_vulkan()
{
    if (vkdev)
    {
...
void Crosshatch_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float threshold, float fadeEdge) const
{
    ...
    constants[17].f = fadeEdge;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Crosshatch_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float threshold, float fadeEdge) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, threshold, fadeEdge);
