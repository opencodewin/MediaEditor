...
\n\
    ...
    { \n\
        vec2 offset = point + dir * sin(dist * p.amplitude - p.progress * p.speed); \n\
        offset = clamp(offset, vec2(0.f, 0.f), vec2(1.f, 1.f)); \n\
        sfpvec4 rgba_from = load_rgba(int(offset.x * (p.w - 1)), int((1.f - offset.y) * (p.h - 1)), p.w, p.h, p.cstep, p.in_format, p.in_type); \n\
        return mix(rgba_from, rgba_to, sfp(p.progress)); \n\
    } \n\
} \n\
\n\
...
static const char WaterDrop_data[] = 
SHADER_HEADER
SHADER_PARAM
