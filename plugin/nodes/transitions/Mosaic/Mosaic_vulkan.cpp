...
#include "Mosaic_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Mosaic_vulkan::Mosaic_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Mosaic");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Mosaic_data, opt, spirv_data) == 0)
    {
...
void Mosaic_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, int endx, int endy) const
{
...
double Mosaic_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, int endx, int endy) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, endx, endy);
