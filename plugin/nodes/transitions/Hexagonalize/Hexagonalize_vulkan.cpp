...
#include "Hexagonalize_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Hexagonalize_vulkan::Hexagonalize_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Hexagonalize");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Hexagonalize_data, opt, spirv_data) == 0)
    {
...
Hexagonalize_vulkan::~Hexagonalize_vulkan()
{
    if (vkdev)
    {
...
void Hexagonalize_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float horizontalHexagons, int steps) const
{
    ...
    constants[16].f = horizontalHexagons;
    constants[17].i = steps;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Hexagonalize_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float horizontalHexagons, int steps) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, horizontalHexagons, steps);
