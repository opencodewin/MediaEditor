...
#define SHADER_MAIN \
" \n\
sfpvec3 bgcolor = sfpvec3(sfp(p.red_back), sfp(p.green_back), sfp(p.blue_back)); \n\
...
{ \n\
    ...
    return mix( \n\
        rgba_from + sfpvec4(sfp(p.progress) * bgcolor, sfp(1.0)), \n\
        rgba_to + sfpvec4(sfp(1.0 - p.progress) * bgcolor, sfp(1.0)), \n\
        sfp(p.progress) \n\
    ); \n\
} \n\
\n\
