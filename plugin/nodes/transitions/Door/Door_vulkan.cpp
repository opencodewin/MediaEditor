...
#include "Door_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Door_vulkan::Door_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Door");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Door_data, opt, spirv_data) == 0)
    {
...
void Door_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, bool open, bool horizon) const
{
    ...
    constants[16].i = open ? 1 : 0;
    constants[17].i = horizon ? 1 : 0;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Door_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, bool open, bool horizon) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, open, horizon);
