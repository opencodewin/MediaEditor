...
{ \n\
    float regress = p.open == 1 ? 1.0 - p.progress : p.progress; \n\
\n\
    float s = 2.0 - abs(((p.horizon == 1 ? uv.y : uv.x) - 0.5) / (regress - 1.0)) - 2.0 * regress; \n\
\n\
    ...
    return mix( \n\
        rgba_from, \n\
        rgba_to, \n\
        sfp(smoothstep(p.open == 1 ? 0.0 : 1.0, p.open == 1 ? 1.0 : 0.0, s)) \n\
    ); \n\
} \n\
\n\
void main() \n\
