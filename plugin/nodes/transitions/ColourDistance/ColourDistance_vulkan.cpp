...
#include "ColourDistance_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
ColourDistance_vulkan::ColourDistance_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "ColourDistance");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(ColourDistance_data, opt, spirv_data) == 0)
    {
...
ColourDistance_vulkan::~ColourDistance_vulkan()
{
    if (vkdev)
    {
...
void ColourDistance_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float power) const
{
    ...
    constants[16].f = power;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double ColourDistance_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float power) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, power);
