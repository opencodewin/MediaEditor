...
{ \n\
    sfpvec4 fTex = load_rgba(int(point.x * (p.w - 1)), int((1.f - point.y) * (p.h - 1)), p.w, p.h, p.cstep, p.in_format, p.in_type); \n\
    sfpvec4 tTex = load_rgba_src2(int(point.x * (p.w2 - 1)), int((1.f - point.y) * (p.h2 - 1)), p.w2, p.h2, p.cstep2, p.in_format2, p.in_type2); \n\
    sfp m = step(distance(fTex, tTex), sfp(p.progress)); \n\
    return mix( \n\
        mix(fTex, tTex, m), \n\
        tTex, \n\
        pow(sfp(p.progress), sfp(p.power)) \n\
    ); \n\
} \n\
void main() \n\
{ \n\
...
static const char ColourDistance_data[] = 
SHADER_HEADER
SHADER_PARAM
