...
#include "RandomSquares_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
RandomSquares_vulkan::RandomSquares_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "RandomSquares");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(RandomSquares_data, opt, spirv_data) == 0)
    {
...
RandomSquares_vulkan::~RandomSquares_vulkan()
{
    if (vkdev)
    {
...
void RandomSquares_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float smoothness, int size) const
{
...
double RandomSquares_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float smoothness, int size) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, smoothness, size);
