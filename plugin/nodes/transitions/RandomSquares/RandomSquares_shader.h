...
#define SHADER_MAIN \
" \n\
ivec2 size = ivec2(p.size, p.size); \n\
float rand(vec2 co) \n\
{ \n\
...
{ \n\
    float r = rand(floor(vec2(size) * point)); \n\
    float m = smoothstep(0.0, -p.smoothness, r - (p.progress * (1.0 + p.smoothness))); \n\
...
static const char RandomSquares_data[] = 
SHADER_HEADER
SHADER_PARAM
