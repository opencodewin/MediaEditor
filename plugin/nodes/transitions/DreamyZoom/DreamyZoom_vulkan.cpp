...
#include "DreamyZoom_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
DreamyZoom_vulkan::DreamyZoom_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "DreamyZoom");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(DreamyZoom_data, opt, spirv_data) == 0)
    {
...
DreamyZoom_vulkan::~DreamyZoom_vulkan()
{
    if (vkdev)
    {
...
void DreamyZoom_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float rotation, float scale) const
{
    ...
    constants[16].f = rotation;
    constants[17].f = scale;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double DreamyZoom_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float rotation, float scale) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, rotation, scale);
