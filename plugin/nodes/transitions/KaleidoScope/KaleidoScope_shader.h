...
{ \n\
    ...
    vec2 q = point; \n\
    float t = pow(p.progress, p.power) * p.speed; \n\
    point = point - 0.5; \n\
    for (int i = 0; i < 7; i++) \n\
    { \n\
        point = vec2(sin(t) * point.x + cos(t) * point.y, sin(t) * point.y - cos(t) * point.x); \n\
        t += p.angle; \n\
        point = abs(mod(point, 2.0) - 1.0); \n\
    } \n\
\n\
...
static const char KaleidoScope_data[] = 
SHADER_HEADER
SHADER_PARAM
