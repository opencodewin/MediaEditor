...
#include "MultiplyBlend_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
MultiplyBlend_vulkan::MultiplyBlend_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "MultiplyBlend");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(MultiplyBlend_data, opt, spirv_data) == 0)
    {
...
MultiplyBlend_vulkan::~MultiplyBlend_vulkan()
{
    if (vkdev)
    {
...
void MultiplyBlend_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress) const
{
...
double MultiplyBlend_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
