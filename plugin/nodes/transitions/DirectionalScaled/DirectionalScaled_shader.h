...
{ \n\
    float y = pow(sin(x * 3.14159265359), 1.); \n\
    return y; \n\
} \n\
sfpvec4 transition (vec2 uv) \n\
{ \n\
    vec2 direction = vec2(p.direction_x, p.direction_y); \n\
    float easedProgress = pow(sin(p.progress * 3.14159265359 / 2.), 3.); \n\
    vec2 point = uv + easedProgress * sign(direction); \n\
    vec2 f = fract(point); \n\
\n\
    float s = 1. - (1. - (1. / p.scale)) * parabola(p.progress); \n\
    f = (f - 0.5) * s  + 0.5; \n\
\n\
    float mixer = step(0.0, point.y) * step(point.y, 1.0) * step(0.0, point.x) * step(point.x, 1.0); \n\
    sfpvec4 rgba_from = load_rgba(int(f.x * (p.w - 1)), int((1.f - f.y) * (p.h - 1)), p.w, p.h, p.cstep, p.in_format, p.in_type); \n\
    sfpvec4 rgba_to = load_rgba_src2(int(f.x * (p.w2 - 1)), int((1.f - f.y) * (p.h2 - 1)), p.w2, p.h2, p.cstep2, p.in_format2, p.in_type2); \n\
\n\
    sfpvec4 col = mix(rgba_to, rgba_from, sfp(mixer)); \n\
\n\
...
static const char DirectionalScaled_data[] = 
SHADER_HEADER
SHADER_PARAM
