...
#include "CrazyParametric_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
CrazyParametric_vulkan::CrazyParametric_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "CrazyParametric");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(CrazyParametric_data, opt, spirv_data) == 0)
    {
...
CrazyParametric_vulkan::~CrazyParametric_vulkan()
{
    if (vkdev)
    {
...
void CrazyParametric_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float amplitude, float smoothness, float pa, float pb) const
{
    ...
    constants[17].f = smoothness;
    constants[18].f = pa;
    constants[19].f = pb;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double CrazyParametric_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float amplitude, float smoothness, float pa, float pb) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, amplitude, smoothness, pa, pb);
