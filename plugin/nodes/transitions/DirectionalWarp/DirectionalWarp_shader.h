...
{ \n\
    ...
    float m = 1.0 - smoothstep(-p.smoothness, 0.0, v.x * uv.x + v.y * uv.y - (d - 0.5 + p.progress * (1.0 + p.smoothness))); \n\
    vec2 from_point = clamp((uv - 0.5) * (1.0 - m) + 0.5, vec2(0.f, 0.f), vec2(1.f, 1.f)); \n\
    vec2 to_point = clamp((uv - 0.5) * m + 0.5, vec2(0.f, 0.f), vec2(1.f, 1.f)); \n\
...
static const char DirectionalWarp_data[] = 
SHADER_HEADER
SHADER_PARAM
