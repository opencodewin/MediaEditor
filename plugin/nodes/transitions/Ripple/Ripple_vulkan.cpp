...
#include "Ripple_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Ripple_vulkan::Ripple_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Ripple");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Ripple_data, opt, spirv_data) == 0)
    {
...
void Ripple_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float amplitude, float speed) const
{
    ...
    constants[17].f = speed;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Ripple_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float amplitude, float speed) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, amplitude, speed);
