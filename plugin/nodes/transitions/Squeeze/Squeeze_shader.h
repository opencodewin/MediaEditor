...
{ \n\
    ...
    if (y < 0.0 || y > 1.0) \n\
    { \n\
        return load_rgba_src2(int(uv.x * (p.w2 - 1)), int((1.f - uv.y) * (p.h2 - 1)), p.w2, p.h2, p.cstep2, p.in_format2, p.in_type2); \n\
    } \n\
    else \n\
    { \n\
        vec2 fp = vec2(uv.x, y); \n\
        vec2 off = p.progress * vec2(0.0, p.colorSeparation); \n\
        sfpvec4 c = load_rgba(int(fp.x * (p.w - 1)), int((1.f - fp.y) * (p.h - 1)), p.w, p.h, p.cstep, p.in_format, p.in_type); \n\
        vec2 fn = clamp(fp - off, vec2(0.f, 0.f), vec2(1.f, 1.f)); \n\
        sfpvec4 cn = load_rgba(int(fn.x * (p.w - 1)), int((1.f - fn.y) * (p.h - 1)), p.w, p.h, p.cstep, p.in_format, p.in_type); \n\
        vec2 fc = clamp(fp + off, vec2(0.f, 0.f), vec2(1.f, 1.f)); \n\
        sfpvec4 cp = load_rgba(int(fc.x * (p.w - 1)), int((1.f - fc.y) * (p.h - 1)), p.w, p.h, p.cstep, p.in_format, p.in_type); \n\
