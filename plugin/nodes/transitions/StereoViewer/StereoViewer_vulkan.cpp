...
#include "StereoViewer_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
StereoViewer_vulkan::StereoViewer_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "StereoViewer");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(StereoViewer_data, opt, spirv_data) == 0)
    {
...
StereoViewer_vulkan::~StereoViewer_vulkan()
{
    if (vkdev)
    {
...
void StereoViewer_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float zoom, float corner_radius) const
{
    ...
    constants[16].f = zoom;
    constants[17].f = corner_radius;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double StereoViewer_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float zoom, float corner_radius) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, zoom, corner_radius);
