...
#include "BookFlip_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
BookFlip_vulkan::BookFlip_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "BookFlip");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(BookFlip_data, opt, spirv_data) == 0)
    {
...
BookFlip_vulkan::~BookFlip_vulkan()
{
    if (vkdev)
    {
...
void BookFlip_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress) const
{
