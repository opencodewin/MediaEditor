...
#include "Squeeze_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Squeeze_vulkan::Squeeze_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Squeeze");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Squeeze_data, opt, spirv_data) == 0)
    {
...
void Squeeze_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float colorSeparation) const
{
    ...
    constants[16].f = colorSeparation;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Squeeze_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float colorSeparation) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, colorSeparation);
