...
#include "StaticWipe_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
StaticWipe_vulkan::StaticWipe_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "StaticWipe");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(StaticWipe_data, opt, spirv_data) == 0)
    {
...
StaticWipe_vulkan::~StaticWipe_vulkan()
{
    if (vkdev)
    {
...
void StaticWipe_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float span, bool UpToDown) const
{
    ...
    constants[16].f = span;
    constants[17].i = UpToDown ? 1 : 0;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double StaticWipe_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float span, bool UpToDown) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, span, UpToDown);
