...
#include "WaterDrop_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
WaterDrop_vulkan::WaterDrop_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "WaterDrop");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(WaterDrop_data, opt, spirv_data) == 0)
    {
...
WaterDrop_vulkan::~WaterDrop_vulkan()
{
    if (vkdev)
    {
...
void WaterDrop_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float speed, float amplitude) const
{
    ...
    constants[17].f = amplitude;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double WaterDrop_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float speed, float amplitude) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, speed, amplitude);
