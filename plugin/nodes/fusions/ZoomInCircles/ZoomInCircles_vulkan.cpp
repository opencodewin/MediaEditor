...
#include "ZoomInCircles_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
ZoomInCircles_vulkan::ZoomInCircles_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "ZoomInCircles");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(ZoomInCircles_data, opt, spirv_data) == 0)
    {
...
ZoomInCircles_vulkan::~ZoomInCircles_vulkan()
{
    if (vkdev)
    {
...
void ZoomInCircles_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress) const
{
...
double ZoomInCircles_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
