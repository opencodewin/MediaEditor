...
#include "Perlin_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Perlin_vulkan::Perlin_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Perlin");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Perlin_data, opt, spirv_data) == 0)
    {
...
void Perlin_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float scale, float smoothness, float seed) const
{
    ...
    constants[16].f = scale;
    constants[17].f = smoothness;
    constants[18].f = seed;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Perlin_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float scale, float smoothness, float seed) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, scale, smoothness, seed);
