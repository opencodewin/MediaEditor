...
#include "Swap_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Swap_vulkan::Swap_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Swap");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Swap_data, opt, spirv_data) == 0)
    {
...
void Swap_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float reflection, float perspective, float depth) const
{
...
double Swap_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float reflection, float perspective, float depth) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
