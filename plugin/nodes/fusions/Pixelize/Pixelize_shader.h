...
#define SHADER_MAIN \
" \n\
ivec2 squaresMin = ivec2(p.size, p.size); \n\
float d = min(p.progress, 1.0 - p.progress); \n\
float dist = p.steps > 0 ? ceil(d * float(p.steps)) / float(p.steps) : d; \n\
vec2 squareSize = 2.0 * dist / vec2(squaresMin); \n\
sfpvec4 transition(vec2 uv) \n\
{ \n\
    vec2 point = dist > 0.0 ? (floor(uv / squareSize) + 0.5) * squareSize : uv; \n\
...
static const char Pixelize_data[] = 
SHADER_HEADER
SHADER_PARAM
