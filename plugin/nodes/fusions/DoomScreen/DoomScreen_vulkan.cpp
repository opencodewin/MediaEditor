...
#include "DoomScreen_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
DoomScreen_vulkan::DoomScreen_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "DoomScreen");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(DoomScreen_data, opt, spirv_data) == 0)
    {
...
DoomScreen_vulkan::~DoomScreen_vulkan()
{
    if (vkdev)
    {
...
void DoomScreen_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float amplitude, float noise, float frequency, float dripScale, int bars) const
{
    ...
    constants[16].f = amplitude;
    constants[17].f = noise;
    constants[18].f = frequency;
    constants[19].f = dripScale;
    constants[20].i = bars;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double DoomScreen_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float amplitude, float noise, float frequency, float dripScale, int bars) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, amplitude, noise, frequency, dripScale, bars);
