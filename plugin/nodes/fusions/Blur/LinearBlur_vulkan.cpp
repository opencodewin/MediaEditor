...
#include "LinearBlur_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
LinearBlur_vulkan::LinearBlur_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "LinearBlur");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(LinearBlur_data, opt, spirv_data) == 0)
    {
...
LinearBlur_vulkan::~LinearBlur_vulkan()
{
    if (vkdev)
    {
...
void LinearBlur_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float intensity, int passes) const
{
    ...
    constants[16].f = intensity;
    constants[17].i = passes;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double LinearBlur_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float intensity, int passes) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, intensity, passes);
