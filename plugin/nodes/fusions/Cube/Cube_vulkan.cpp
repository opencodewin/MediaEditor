...
namespace ImGui 
{
Cube_vulkan::Cube_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Cube");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Cube_data, opt, spirv_data) == 0)
    {
...
void Cube_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float persp, float unzoom, float reflection, float floating) const
{
    ...
    constants[16].f = persp;
    constants[17].f = unzoom;
    constants[18].f = reflection;
    constants[19].f = floating;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Cube_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float persp, float unzoom, float reflection, float floating) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, persp, unzoom, reflection, floating);
