...
#include "DirectionalWarp_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
DirectionalWarp_vulkan::DirectionalWarp_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "DirectionalWarp");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(DirectionalWarp_data, opt, spirv_data) == 0)
    {
...
DirectionalWarp_vulkan::~DirectionalWarp_vulkan()
{
    if (vkdev)
    {
...
void DirectionalWarp_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float smoothness, float direction_x, float direction_y) const
{
    ...
    constants[16].f = smoothness;
    constants[17].f = direction_x;
    constants[18].f = direction_y;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double DirectionalWarp_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float smoothness, float direction_x, float direction_y) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, smoothness, direction_x, direction_y);
