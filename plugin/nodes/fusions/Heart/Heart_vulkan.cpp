...
#include "Heart_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Heart_vulkan::Heart_vulkan(int gpu)
{
    ...
    if (compile_spirv_module(Heart_data, opt, spirv_data) == 0)
    {
...
void Heart_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress) const
{
...
double Heart_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
