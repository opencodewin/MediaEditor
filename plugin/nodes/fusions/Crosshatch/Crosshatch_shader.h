...
{ \n\
    float dist = distance(center, point) / p.threshold; \n\
    float r = p.progress - min(rand(vec2(point.y, 0.0)), rand(vec2(0.0, point.x))); \n\
    ...
    return mix(rgba_from, rgba_to, sfp(mix(0.0, mix(step(dist, r), 1.0, smoothstep(1.0 - p.fadeEdge, 1.0, p.progress)), smoothstep(0.0, p.fadeEdge, p.progress)))); \n\
} \n\
\n\
void main() \n\
{ \n\
...
static const char Crosshatch_data[] = 
SHADER_HEADER
SHADER_PARAM
