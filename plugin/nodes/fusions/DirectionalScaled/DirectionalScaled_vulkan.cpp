...
#include "DirectionalScaled_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
DirectionalScaled_vulkan::DirectionalScaled_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "DirectionalScaled");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(DirectionalScaled_data, opt, spirv_data) == 0)
    {
...
DirectionalScaled_vulkan::~DirectionalScaled_vulkan()
{
    if (vkdev)
    {
...
void DirectionalScaled_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float direction_x, float direction_y, float scale) const
{
    ...
    constants[16].f = direction_x;
    constants[17].f = direction_y;
    constants[18].f = scale;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double DirectionalScaled_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float direction_x, float direction_y, float scale) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, direction_x, direction_y, scale);
