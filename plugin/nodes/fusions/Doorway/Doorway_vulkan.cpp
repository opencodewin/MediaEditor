...
#include "Doorway_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Doorway_vulkan::Doorway_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Doorway");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Doorway_data, opt, spirv_data) == 0)
    {
...
void Doorway_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float reflection, float perspective, float depth) const
{
    ...
    constants[16].f = reflection;
    constants[17].f = perspective;
    constants[18].f = depth;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Doorway_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float reflection, float perspective, float depth) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, reflection, perspective, depth);
