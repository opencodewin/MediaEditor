...
#include "CrossWarp_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
CrossWarp_vulkan::CrossWarp_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "CrossWarp");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(CrossWarp_data, opt, spirv_data) == 0)
    {
...
CrossWarp_vulkan::~CrossWarp_vulkan()
{
    if (vkdev)
    {
...
void CrossWarp_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress) const
{
...
double CrossWarp_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
