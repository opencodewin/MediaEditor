...
{ \n\
    float x = p.progress; \n\
    x = smoothstep(.0, 1.0, (x * 2.0 + point.x - 1.0)); \n\
    vec2 from_point = (point - .5) * (1. - x) + .5; \n\
    from_point = clamp(from_point, vec2(0.f, 0.f), vec2(1.f, 1.f)); \n\
    vec2 to_point = (point - .5) * x + .5; \n\
    ...
    return mix(rgba_from, rgba_to, sfp(x)); \n\
} \n\
\n\
void main() \n\
{ \n\
...
static const char CrossWarp_data[] = 
SHADER_HEADER
SHADER_PARAM
