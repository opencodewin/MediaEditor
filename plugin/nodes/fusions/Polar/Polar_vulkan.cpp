...
#include "Polar_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Polar_vulkan::Polar_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Polar");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Polar_data, opt, spirv_data) == 0)
    {
...
void Polar_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, int segments) const
{
    ...
    constants[16].i = segments;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Polar_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, int segments) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, segments);
