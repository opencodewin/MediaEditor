...
#include "Morph_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Morph_vulkan::Morph_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Morph");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Morph_data, opt, spirv_data) == 0)
    {
...
void Morph_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float strength) const
{
...
double Morph_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float strength) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
