...
#include "Pinwheel_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Pinwheel_vulkan::Pinwheel_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Pinwheel");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Pinwheel_data, opt, spirv_data) == 0)
    {
...
Pinwheel_vulkan::~Pinwheel_vulkan()
{
    if (vkdev)
    {
...
void Pinwheel_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float speed) const
{
...
double Pinwheel_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float speed) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, speed);
