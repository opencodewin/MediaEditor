...
\n\
    float circPos = atan(point.y - 0.5, point.x - 0.5) + p.progress * p.speed; \n\
    float modPos = mod(circPos, 3.1415 / 4.); \n\
    float _signed = sign(p.progress - modPos); \n\
    ...
    return mix(rgba_to, rgba_from, sfp(step(_signed, 0.5))); \n\
} \n\
\n\
void main() \n\
{ \n\
...
static const char Pinwheel_data[] = 
SHADER_HEADER
SHADER_PARAM
