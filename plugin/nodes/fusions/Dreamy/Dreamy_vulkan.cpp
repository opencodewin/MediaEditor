...
#include "Dreamy_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Dreamy_vulkan::Dreamy_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Dreamy");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Dreamy_data, opt, spirv_data) == 0)
    {
...
void Dreamy_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress) const
{
...
double Dreamy_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
