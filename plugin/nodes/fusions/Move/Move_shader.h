...
{ \n\
    vec2 point = uv + p.progress * sign(vec2(p.direction_x, p.direction_y)); \n\
    vec2 f = fract(point); \n\
    return mix( \n\
        load_rgba_src2(int(f.x * (p.w2 - 1)), int((1.f - f.y) * (p.h2 - 1)), p.w2, p.h2, p.cstep2, p.in_format2, p.in_type2), \n\
        load_rgba(int(f.x * (p.w - 1)), int((1.f - f.y) * (p.h - 1)), p.w, p.h, p.cstep, p.in_format, p.in_type), \n\
        sfp(step(0.0, point.y) * step(point.y, 1.0) * step(0.0, point.x) * step(point.x, 1.0)) \n\
    ); \n\
} \n\
\n\
void main() \n\
