...
#include "GridFlip_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
GridFlip_vulkan::GridFlip_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "GridFlip");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(GridFlip_data, opt, spirv_data) == 0)
    {
...
GridFlip_vulkan::~GridFlip_vulkan()
{
    if (vkdev)
    {
...
void GridFlip_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, ImPixel& back_colour, float pause, float dividerWidth, float randomness, int w, int h) const
{
    ...
    std::vector<vk_constant_type> constants(25);
    ...
    constants[20].f = pause;
    constants[21].f = dividerWidth;
    constants[22].f = randomness;
    constants[23].i = w;
    constants[24].i = h;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double GridFlip_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, ImPixel& back_colour, float pause, float dividerWidth, float randomness, int w, int h) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, back_colour, pause, dividerWidth, randomness, w, h);
