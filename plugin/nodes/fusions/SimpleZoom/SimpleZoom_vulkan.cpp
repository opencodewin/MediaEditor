...
#include "SimpleZoom_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
SimpleZoom_vulkan::SimpleZoom_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "SimpleZoom");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(SimpleZoom_data, opt, spirv_data) == 0)
    {
...
SimpleZoom_vulkan::~SimpleZoom_vulkan()
{
    if (vkdev)
    {
...
void SimpleZoom_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float quickness) const
{
...
double SimpleZoom_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float quickness) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, quickness);
