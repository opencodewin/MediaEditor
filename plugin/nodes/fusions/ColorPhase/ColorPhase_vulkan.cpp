...
#include "ColorPhase_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
ColorPhase_vulkan::ColorPhase_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "ColorPhase");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(ColorPhase_data, opt, spirv_data) == 0)
    {
...
ColorPhase_vulkan::~ColorPhase_vulkan()
{
    if (vkdev)
    {
...
void ColorPhase_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, ImPixel& from_colour, ImPixel& to_colour) const
{
    ...
    std::vector<vk_constant_type> constants(24);
    ...
    constants[16].f = from_colour.r;
    constants[17].f = from_colour.g;
    constants[18].f = from_colour.b;
    constants[19].f = from_colour.a;
    constants[20].f = to_colour.r;
    constants[21].f = to_colour.g;
    constants[22].f = to_colour.b;
    constants[23].f = to_colour.a;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double ColorPhase_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, ImPixel& from_colour, ImPixel& to_colour) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, from_colour, to_colour);
