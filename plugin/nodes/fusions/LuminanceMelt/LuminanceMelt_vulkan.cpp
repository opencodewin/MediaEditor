...
#include "LuminanceMelt_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
LuminanceMelt_vulkan::LuminanceMelt_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "LuminanceMelt");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(LuminanceMelt_data, opt, spirv_data) == 0)
    {
...
LuminanceMelt_vulkan::~LuminanceMelt_vulkan()
{
    if (vkdev)
    {
...
void LuminanceMelt_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float threshold, int direction, int above) const
{
    ...
    constants[16].f = threshold;
    constants[17].i = direction;
    constants[18].i = above;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double LuminanceMelt_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float threshold, int direction, int above) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, threshold, direction, above);
