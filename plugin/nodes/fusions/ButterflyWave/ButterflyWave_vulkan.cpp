...
#include "ButterflyWave_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
ButterflyWave_vulkan::ButterflyWave_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "ButterflyWave");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(ButterflyWave_data, opt, spirv_data) == 0)
    {
...
ButterflyWave_vulkan::~ButterflyWave_vulkan()
{
    if (vkdev)
    {
...
void ButterflyWave_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float amplitude, float waves, float colorSeparation) const
{
    ...
    constants[17].f = waves;
    constants[18].f = colorSeparation;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double ButterflyWave_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float amplitude, float waves, float colorSeparation) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, amplitude, waves, colorSeparation);
