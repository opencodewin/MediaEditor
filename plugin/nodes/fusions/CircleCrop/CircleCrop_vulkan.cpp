...
#include "CircleCrop_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
CircleCrop_vulkan::CircleCrop_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "CircleCrop");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(CircleCrop_data, opt, spirv_data) == 0)
    {
...
CircleCrop_vulkan::~CircleCrop_vulkan()
{
    if (vkdev)
    {
...
void CircleCrop_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, ImPixel& back_colour) const
{
...
double CircleCrop_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, ImPixel& back_colour) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
