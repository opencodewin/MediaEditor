...
#include "KaleidoScope_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
KaleidoScope_vulkan::KaleidoScope_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "KaleidoScope");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(KaleidoScope_data, opt, spirv_data) == 0)
    {
...
KaleidoScope_vulkan::~KaleidoScope_vulkan()
{
    if (vkdev)
    {
...
void KaleidoScope_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, float speed, float angle, float power) const
{
    ...
    constants[16].f = speed;
    constants[17].f = angle;
    constants[18].f = power;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double KaleidoScope_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, float speed, float angle, float power) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, speed, angle, power);
