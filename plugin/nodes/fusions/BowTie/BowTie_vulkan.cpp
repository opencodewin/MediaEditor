...
#include "BowTie_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
BowTieHorizontal_vulkan::BowTieHorizontal_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "BowTie Horizontal");

    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(BowTie_data, opt, spirv_data) == 0)
    {
...
BowTieHorizontal_vulkan::~BowTieHorizontal_vulkan()
{
    if (vkdev)
    {
...
void BowTieHorizontal_vulkan::upload_param(const VkMat& src1, const VkMat& src2, VkMat& dst, float progress, int direction) const
{
    ...
    constants[16].i = direction;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double BowTieHorizontal_vulkan::transition(const ImMat& src1, const ImMat& src2, ImMat& dst, float progress, int direction) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src1_gpu, src2_gpu, dst_gpu, progress, direction);
