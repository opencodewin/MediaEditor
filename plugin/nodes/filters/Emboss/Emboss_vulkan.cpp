...
#include "Emboss_shader.h"
#include "ImVulkanShader.h"

namespace ImGui
{
Emboss_vulkan::Emboss_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Emboss");
...
void Emboss_vulkan::upload_param(const VkMat& src, VkMat& dst, float intensity, float angle, int stride)
{
    ...
    constants[10].f = intensity;
    constants[11].f = angle * M_PI / 180.f;
    constants[12].i = stride;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Emboss_vulkan::filter(const ImMat& src, ImMat& dst, float intensity, float angle, int stride)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, intensity, angle, stride);
