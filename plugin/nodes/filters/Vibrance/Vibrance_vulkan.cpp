...
#include "Vibrance_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Vibrance_vulkan::Vibrance_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Vibrance");
...
Vibrance_vulkan::~Vibrance_vulkan()
{
    if (vkdev)
    {
...
void Vibrance_vulkan::upload_param(const VkMat& src, VkMat& dst, float vibrance) const
{
    ...
    constants[10].f = vibrance;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Vibrance_vulkan::filter(const ImMat& src, ImMat& dst, float vibrance) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, vibrance);
