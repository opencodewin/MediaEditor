...
#include "Kuwahara_shader.h"
#include "ImVulkanShader.h"

namespace ImGui
{
Kuwahara_vulkan::Kuwahara_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Kuwahara");
...
Kuwahara_vulkan::~Kuwahara_vulkan()
{
    if (vkdev)
    {
...
void Kuwahara_vulkan::upload_param(const VkMat& src, VkMat& dst, float scale)
{
...
double Kuwahara_vulkan::effect(const ImMat& src, ImMat& dst, float scale)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
