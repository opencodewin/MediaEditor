...
#include "BarrelDistortion_shader.h"
#include "ImVulkanShader.h"

namespace ImGui
{
BarrelDistortion_vulkan::BarrelDistortion_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "BarrelDistortion");
...
BarrelDistortion_vulkan::~BarrelDistortion_vulkan()
{
    if (vkdev)
    {
...
void BarrelDistortion_vulkan::upload_param(const VkMat& src, VkMat& dst, float scale, float pow)
{
...
double BarrelDistortion_vulkan::effect(const ImMat& src, ImMat& dst, float scale, float pow)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
