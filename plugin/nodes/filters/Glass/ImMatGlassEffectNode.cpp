...
struct GlassEffectNode final : Node
{
    ...
    GlassEffectNode(BP* blueprint): Node(blueprint) { m_Name = "Glass Blur"; }
    ...
    bool DrawCustomLayout(ImGuiContext * ctx, float zoom, ImVec2 origin, ImGui::ImCurveEdit::keys * key, bool embedded) override
    {
        ...
        if (key) ImGui::ImCurveCheckEditKeyWithID("##add_curve_scale##Glass", key, m_ScaleIn.IsLinked(), "scale##Glass@" + std::to_string(m_ID), 0.0f, 30.f, 20.f, m_ScaleIn.m_ID);
        ImGui::EndDisabled();
        ImGui::PopItemWidth();
        ImGui::PopStyleColor();
