...
void Sobel_vulkan::upload_param(const VkMat& src, VkMat& dst, float edgeStrength)
{
    ...
    constants[10].f = edgeStrength;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Sobel_vulkan::filter(const ImMat& src, ImMat& dst, float edgeStrength)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, edgeStrength);
