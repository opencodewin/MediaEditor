...
namespace ImGui
{
Sketch_vulkan::Sketch_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Sketch");
...
void Sketch_vulkan::upload_param(const VkMat& src, VkMat& dst, float intensity, int step)
{
...
double Sketch_vulkan::filter(const ImMat& src, ImMat& dst, float intensity, int step)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, intensity, step);
