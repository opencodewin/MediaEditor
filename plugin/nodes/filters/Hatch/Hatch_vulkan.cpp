...
#include "Hatch_shader.h"
#include "ImVulkanShader.h"

namespace ImGui
{
Hatch_vulkan::Hatch_vulkan(int gpu)
{
...
void Hatch_vulkan::upload_param(const VkMat& src, VkMat& dst, float spacing, float width)
{
    ...
    constants[10].f = spacing;
    constants[11].f = width;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Hatch_vulkan::filter(const ImMat& src, ImMat& dst, float spacing, float width)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, spacing, width);
