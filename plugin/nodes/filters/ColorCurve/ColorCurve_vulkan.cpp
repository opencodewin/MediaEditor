...
#include "ColorCurve_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
ColorCurve_vulkan::ColorCurve_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "ColorCurve");
...
ColorCurve_vulkan::~ColorCurve_vulkan()
{
    if (vkdev)
    {
...
void ColorCurve_vulkan::upload_param(const VkMat& src, VkMat& dst, const VkMat& curve) const
{
    ...
    constants[10].i = curve.w;
    constants[11].i = curve.c;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double ColorCurve_vulkan::filter(const ImMat& src, ImMat& dst, const ImMat& curve) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    VkMat curve_gpu;
    if (curve.device == IM_DD_VULKAN)
    {
        curve_gpu = curve;
    }
    else if (curve.device == IM_DD_CPU)
    {
        cmd->record_clone(curve, curve_gpu, opt);
    }
    ...
    upload_param(src_gpu, dst_gpu, curve_gpu);
