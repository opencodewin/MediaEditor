...
#include "SmartDenoise_shader.h"
#include "ImVulkanShader.h"

namespace ImGui
{
SmartDenoise_vulkan::SmartDenoise_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Smart Denoise");
...
SmartDenoise_vulkan::~SmartDenoise_vulkan()
{
    if (vkdev)
    {
...
void SmartDenoise_vulkan::upload_param(const VkMat& src, VkMat& dst, float sigma, float kSigma, float threshold)
{
    ...
    constants[10].f = sigma;
    constants[11].f = kSigma;
    constants[12].f = threshold;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double SmartDenoise_vulkan::filter(const ImMat& src, ImMat& dst, float sigma, float kSigma, float threshold)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, sigma, kSigma, threshold);
