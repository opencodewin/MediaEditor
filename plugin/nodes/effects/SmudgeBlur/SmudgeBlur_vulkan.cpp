...
#include "SmudgeBlur_shader.h"
#include "ImVulkanShader.h"

namespace ImGui
{
SmudgeBlur_vulkan::SmudgeBlur_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "SmudgeBlur");
...
SmudgeBlur_vulkan::~SmudgeBlur_vulkan()
{
    if (vkdev)
    {
...
void SmudgeBlur_vulkan::upload_param(const VkMat& src, VkMat& dst, float radius, float iterations)
{
    ...
    constants[11].f = iterations;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double SmudgeBlur_vulkan::effect(const ImMat& src, ImMat& dst, float radius, float iterations)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, radius, iterations);
