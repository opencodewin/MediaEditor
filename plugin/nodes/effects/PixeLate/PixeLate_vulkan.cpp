...
namespace ImGui
{
PixeLate_vulkan::PixeLate_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "PixeLate");
...
PixeLate_vulkan::~PixeLate_vulkan()
{
    if (vkdev)
    {
...
void PixeLate_vulkan::upload_param(const VkMat& src, VkMat& dst, float radius)
{
...
double PixeLate_vulkan::effect(const ImMat& src, ImMat& dst, float radius)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, radius);
