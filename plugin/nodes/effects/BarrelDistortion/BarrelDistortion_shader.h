...
{ \n\
    ...
    vec2 center = 2.0 * uv - vec2(1.0); \n\
    float barrel = min(1.0 - length(center) * p.scale, 1.0) * p.pow; \n\
    vec2 pos = uv - center * barrel; \n\
    sfpvec4 rgba = load_rgba(int(pos.x * (p.w - 1)), int(pos.y * (p.h - 1)), p.w, p.h, p.cstep, p.in_format, p.in_type); \n\
