...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // add by Dicky
    if (style.DoButtonBorder && style.ButtonBorderSize > 0)
    {
        ImU32 col_light = ColorBrightnessU32(col, 1.5);
        ImU32 col_dark = ColorBrightnessU32(col, 0.5);
        if (style.FrameRounding > 0)
        {
            ImVec2 ptl0 = bb.Min + ImVec2(style.FrameRounding * 0.5, 0);
            ImVec2 ptl1 = bb.Min + ImVec2(0, style.FrameRounding * 0.5);
            ImVec2 ptr0 = ImVec2(bb.Max.x - style.FrameRounding * 0.5, bb.Min.y);
            ImVec2 ptr1 = ImVec2(bb.Max.x, bb.Min.y + style.FrameRounding * 0.5);
            ImVec2 pbl0 = ImVec2(bb.Min.x, bb.Max.y - style.FrameRounding * 0.5);
            ImVec2 pbl1 = ImVec2(bb.Min.x + style.FrameRounding * 0.5, bb.Max.y);
            ImVec2 pbr0 = bb.Max - ImVec2(0, style.FrameRounding * 0.5);
            ImVec2 pbr1 = bb.Max - ImVec2(style.FrameRounding * 0.5, 0);
            window->DrawList->AddLine(ptl0, ptr0, col_light, style.ButtonBorderSize);
            window->DrawList->AddLine(ptl1, pbl0, col_light, style.ButtonBorderSize);
            window->DrawList->AddLine(ptr1, pbr0, col_dark, style.ButtonBorderSize);
            window->DrawList->AddLine(pbl1, pbr1, col_dark, style.ButtonBorderSize);
            window->DrawList->PathArcToFast(bb.Min + ImVec2(style.FrameRounding * 0.5 + 1, style.FrameRounding * 0.5 + 1), style.FrameRounding * 0.5, 6, 9);
            window->DrawList->PathStroke(col_light, ImDrawFlags_None, style.ButtonBorderSize);
            window->DrawList->PathArcToFast(bb.Min + ImVec2(style.FrameRounding * 0.5 + 1, bb.GetHeight() - style.FrameRounding * 0.5 - 1), style.FrameRounding * 0.5, 6, 3);
            window->DrawList->PathStroke(col_light, ImDrawFlags_None, style.ButtonBorderSize);
            window->DrawList->PathArcToFast(bb.Max - ImVec2(style.FrameRounding * 0.5, bb.GetHeight() - style.FrameRounding * 0.5), style.FrameRounding * 0.5, 9, 12);
            window->DrawList->PathStroke(col_dark, ImDrawFlags_None, style.ButtonBorderSize);
            window->DrawList->PathArcToFast(bb.Max - ImVec2(style.FrameRounding * 0.5, style.FrameRounding * 0.5), style.FrameRounding * 0.5, 0, 3);
            window->DrawList->PathStroke(col_dark, ImDrawFlags_None, style.ButtonBorderSize);
        }
        else
        {
            window->DrawList->AddLine(bb.Min, ImVec2(bb.Max.x, bb.Min.y), col_light, style.ButtonBorderSize);
            window->DrawList->AddLine(bb.Min, ImVec2(bb.Min.x, bb.Max.y), col_light, style.ButtonBorderSize);
            window->DrawList->AddLine(ImVec2(bb.Max.x, bb.Min.y), bb.Max, col_dark, style.ButtonBorderSize);
            window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Max.y), bb.Max, col_dark, style.ButtonBorderSize);
        }
    }
    // add by Dicky end
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    if (style.DoButtonBorder && style.ButtonBorderSize > 0)
    {
        ...
        ImU32 col_light = active ? ColorBrightnessU32(GetColorU32(ImGuiCol_CheckMark), 0.75) : ColorBrightnessU32(col, 1.5);
        ImU32 col_dark = ColorBrightnessU32(col, 0.1);
        window->DrawList->AddCircle(center, radius + 1, col, num_segment, 1);
        window->DrawList->PathArcTo(center, radius - 1, ImDegToRad(135.f), ImDegToRad(315.f));
        window->DrawList->PathStroke(col_dark, ImDrawFlags_None, style.ButtonBorderSize);
        window->DrawList->PathArcTo(center, radius - 1, -ImDegToRad(45.f), ImDegToRad(135.f));
        window->DrawList->PathStroke(col_light, ImDrawFlags_None, style.ButtonBorderSize);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    {
        // modify by Dicky
        if (style.FrameShading > 0.0f) {
            ImVec4 fill_colPre = ImGui::ColorConvertU32ToFloat4(frame_col);
            fill_colPre.w *= 1.0f - style.FrameShading;
            ImU32 fill_col1 = ImGui::ColorConvertFloat4ToU32(fill_colPre);
            window->DrawList->AddRectFilledMultiColor(bb.Min, ImVec2(value_x2, bb.Max.y), frame_col, frame_col, fill_col1, fill_col1, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);
        } else {
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    {
        float height = frame_bb.GetHeight() / 4;
        ImRect slider_bb = frame_bb;
        slider_bb.Expand(ImVec2(0, -height));
        RenderFrame(slider_bb.Min, slider_bb.Max, IM_COL32_ALPHA(frame_col, 255), true, g.Style.FrameRounding);
    }
    else
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ...
        else if (flags & ImGuiSliderFlags_Mark)
        {
            ...
            grab_bb.Expand(ImVec2(16, 2));
            if (g.Style.FrameShading > 0.0f) {
                ImU32 fill_col = GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab);
                ImVec4 fill_colPre = ImGui::ColorConvertU32ToFloat4(fill_col);
                fill_colPre.w *= 1.0f - g.Style.FrameShading;
                ImU32 fill_col1 = ImGui::ColorConvertFloat4ToU32(fill_colPre);
                window->DrawList->AddRectFilledMultiColor(grab_bb.Min, grab_bb.Max, fill_col, fill_col, fill_col1, fill_col1, style.GrabRounding);
            } else {
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    {
        float width = frame_bb.GetWidth() / 4;
        ImRect slider_bb = frame_bb;
        slider_bb.Expand(ImVec2(-width, 0));
        RenderFrame(slider_bb.Min, slider_bb.Max, IM_COL32_ALPHA(frame_col, 255), true, g.Style.FrameRounding);
    }
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // Render frame
    if (!is_multiline || (is_multiline && style.DoFrameShadingForMultilineText)) // modify by Dicky
    {
        RenderNavHighlight(frame_bb, id);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            // modify by Dicky, disable text internationalize when user input
