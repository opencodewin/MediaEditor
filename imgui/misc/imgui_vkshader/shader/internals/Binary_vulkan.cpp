...
namespace ImGui 
{
Binary_vulkan::Binary_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Binary");
...
void Binary_vulkan::upload_param(const VkMat& src, VkMat& dst, float v_min, float v_max)
{
    ...
    constants[10].f = v_min;
    constants[11].f = v_max;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Binary_vulkan::forward(const ImMat& src, ImMat& dst, float v_min, float v_max)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    auto color_format = src.color_format;
    int channels = IM_ISALPHA(color_format) ? 4 : IM_ISRGB(color_format) ? 3 : IM_ISMONO(color_format) ? 1 : 4;
    VkMat dst_gpu;
    dst_gpu.create_type(src.w, src.h, channels, src.type, opt.blob_vkallocator);
    ...
    upload_param(src_gpu, dst_gpu, v_min, v_max);
