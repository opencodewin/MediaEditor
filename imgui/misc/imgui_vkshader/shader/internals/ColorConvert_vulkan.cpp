...
void ColorConvert_vulkan::upload_param(const VkMat& Im, VkMat& dst, int video_shift, ImInterpolateMode type) const
{
    std::vector<VkMat> bindings(8);
    if      (dst.type == IM_DT_INT8)     bindings[0] = dst;
    else if (dst.type == IM_DT_INT16 || dst.type == IM_DT_INT16_BE)    bindings[1] = dst;
...
double ColorConvert_vulkan::GRAY2RGBA(const ImMat& im, ImMat & im_RGB, int video_shift, ImInterpolateMode type) const
{
    double ret = -1.f;
    ...
    VkMat dst_gpu;
    dst_gpu.create_type(im_RGB.w > 0 ? im_RGB.w : im.w, im_RGB.h > 0 ? im_RGB.h : im.h, 4, im_RGB.type, opt.blob_vkallocator);
    ...
    upload_param(src_gpu, dst_gpu, video_shift, type);
