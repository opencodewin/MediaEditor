...
#include "Dilation_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Dilation_vulkan::Dilation_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Dilation");
...
Dilation_vulkan::~Dilation_vulkan()
{
    if (vkdev)
    {
...
void Dilation_vulkan::upload_param(const VkMat& src, VkMat& dst, int ksz)
{
...
double Dilation_vulkan::filter(const ImMat& src, ImMat& dst, int ksz)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
