...
#include "Saturation_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Saturation_vulkan::Saturation_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Saturation");
...
Saturation_vulkan::~Saturation_vulkan()
{
    if (vkdev)
    {
...
void Saturation_vulkan::upload_param(const VkMat& src, VkMat& dst, float saturation) const
{
    ...
    constants[10].f = saturation;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Saturation_vulkan::filter(const ImMat& src, ImMat& dst, float saturation) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
