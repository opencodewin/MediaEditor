...
namespace ImGui
{
Glass_vulkan::Glass_vulkan(int gpu)
{
...
void Glass_vulkan::upload_param(const VkMat& src, VkMat& dst, float scale)
{
...
double Glass_vulkan::effect(const ImMat& src, ImMat& dst, float scale)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
