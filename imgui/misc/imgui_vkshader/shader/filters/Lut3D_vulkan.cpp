...
try_again:
                            ...
                            else if (!strncmp(line, "LUT_3D_INPUT_RANGE", 18)) 
                            {
                                float min_val = 0.0f; float max_val = 1.0f;
                                sscanf(line + 19, "%f %f", &min_val, &max_val);
                                min[0] = min_val; min[1] = min_val; min[2] = min_val;
                                max[0] = max_val; max[1] = max_val; max[2] = max_val;
                                //fprintf(stderr, "min: %f %f %f | max: %f %f %f\n", min[0], min[1], min[2], max[0], max[1], max[2]);
                                goto try_again;
                            }
