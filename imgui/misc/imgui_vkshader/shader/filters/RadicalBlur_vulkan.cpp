...
#include "RadicalBlur_shader.h"
#include "ImVulkanShader.h"

namespace ImGui
{
RadicalBlur_vulkan::RadicalBlur_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "RadicalBlur");
...
RadicalBlur_vulkan::~RadicalBlur_vulkan()
{
    if (vkdev)
    {
...
void RadicalBlur_vulkan::upload_param(const VkMat& src, VkMat& dst, float radius, float dist, float intensity, float count)
{
    ...
    constants[10].f = radius;
    constants[11].f = dist;
    constants[12].f = intensity;
    constants[13].f = count;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double RadicalBlur_vulkan::effect(const ImMat& src, ImMat& dst, float radius, float dist, float intensity, float count)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, radius, dist, intensity, count);
