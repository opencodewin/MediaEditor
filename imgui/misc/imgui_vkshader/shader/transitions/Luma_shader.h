...
{ \n\
    ...
    sfp mask = load_gray_mask(int(uv.x * (p.wm - 1)), int((1.f - uv.y) * (p.hm - 1)), p.wm, p.hm, p.cstepm, p.in_formatm, p.in_typem, 255.f); \n\
    return mix( \n\
        rgba_to, \n\
        rgba_from, \n\
        step(sfp(p.progress), mask) \n\
    ); \n\
} \n\
\n\
void main() \n\
...
layout (binding = 12) readonly buffer mask_int8       { uint8_t   mask_data_int8[]; };
layout (binding = 13) readonly buffer mask_int16      { uint16_t  mask_data_int16[]; };
layout (binding = 14) readonly buffer mask_float16    { float16_t mask_data_float16[]; };
layout (binding = 15) readonly buffer mask_float32    { float     mask_data_float32[]; };
)"
SHADER_LOAD_RGBA
