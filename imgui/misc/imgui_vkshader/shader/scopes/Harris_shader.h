...
{ \n\
    ...
    sfp cornerness = sfp(0.f); \n\
    if (p.noble == 1) \n\
    { \n\
        // R = (Ix^2 * Iy^2 - Ixy * Ixy) / (Ix^2 + Iy^2) \n\
        cornerness = (derivativeElements.x * derivativeElements.y - (zElement * zElement)) / (derivativeSum); \n\
    } \n\
    else \n\
    { \n\
        // R = Ix^2 * Iy^2 - Ixy * Ixy - k * (Ix^2 + Iy^2)^2 \n\
        cornerness = derivativeElements.x * derivativeElements.y - (zElement * zElement) - sfp(p.harris) * derivativeSum * derivativeSum; \n\
    } \n\
