...
#include "../filters/Filter2DS_shader.h"
...
void Harris_vulkan::upload_param(const VkMat& src, VkMat& dst, int _blurRadius, float edgeStrength, float threshold, float harris, float sensitivity, bool isNoble)
{
    ...
    std::vector<vk_constant_type> harris_constants(13);
    ...
    harris_constants[12].i = isNoble ? 1 : 0;
...
double Harris_vulkan::scope(const ImMat& src, ImMat& dst, int _blurRadius, float edgeStrength, float threshold, float harris, float sensitivity, bool isNoble)
{
    double ret = 0.0;
    ...
    upload_param(src_gpu, dst_gpu, _blurRadius, edgeStrength, threshold, harris, sensitivity, isNoble);
