...
{ \n\
    ...
    vec4 _p = mix(vec4(c.bg, K.wz), vec4(c.gb, K.xy), step(c.b, c.g)); \n\
    vec4 q = mix(vec4(_p.xyw, c.r), vec4(c.r, _p.yzx), step(_p.x, c.r)); \n\
    \n\
...
{ \n\
    ...
    vec4 _p = (rgb.g < rgb.b) ? vec4(rgb.bg, -1., 2. / 3.) : vec4(rgb.gb, 0., -1. / 3.); \n\
    vec4 q = (rgb.r < _p.x) ? vec4(_p.xyw, rgb.r) : vec4(rgb.r, _p.yzx); \n\
