...
" \n\
vec2 fish2panorama(int u, int v) \n\
{ \n\
	...
	vec3 psph; \n\
\n\
	float FOV = p.fov / 180.0 * M_PI; // FOV of the fisheye, eg: 180 degrees \n\
	float width = p.w; \n\
	float height = p.h; \n\
\n\
	// Polar angles \n\
	theta = 2.0 * M_PI * (u / 2.0 / width - center.x); // -pi to pi \n\
	phi = M_PI * ((p.out_h - v) / height - center.y);  // -pi/2 to pi/2 \n\
\n\
	// Vector in 3D space \n\
...
\n\
	// Pixel in pano space \n\
	r = radius * phi / FOV;  \n\
	pfish.x = center.x * width + r * cos(theta); \n\
	pfish.y = center.y * height - r * sin(theta); \n\
\n\
	return pfish; \n\
} \n\
"
...
{ \n\
    ...
    if (gx >= p.out_w || gy >= p.out_h) \n\
        return; \n\
    vec2 pfish = fish2panorama(gx, gy); \n\
...
static const char Fish2Panorama_data[] = 
SHADER_HEADER
SHADER_PARAM
