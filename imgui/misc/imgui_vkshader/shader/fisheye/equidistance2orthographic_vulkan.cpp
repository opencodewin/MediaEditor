...
#include "equidistance2orthographic_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Equidistance2Orthographic_vulkan::Equidistance2Orthographic_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Equidistance2Orthographic");
    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Equidistance2Orthographic_data, opt, spirv_data) == 0)
    {
...
Equidistance2Orthographic_vulkan::~Equidistance2Orthographic_vulkan()
{
    if (vkdev)
    {
...
void Equidistance2Orthographic_vulkan::upload_param(const VkMat& src, VkMat& dst, float fov, float cx, float cy, ImInterpolateMode mode)
{
...
double Equidistance2Orthographic_vulkan::filter(const ImMat& src, ImMat& dst, float fov, float cx, float cy, ImInterpolateMode mode)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
