#include "orthographic2equidistance_vulkan.h"
#include "orthographic2equidistance_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Orthographic2Equidistance_vulkan::Orthographic2Equidistance_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Orthographic2Equidistance");
    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Orthographic2Equidistance_data, opt, spirv_data) == 0)
    {
...
Orthographic2Equidistance_vulkan::~Orthographic2Equidistance_vulkan()
{
    if (vkdev)
    {
...
void Orthographic2Equidistance_vulkan::upload_param(const VkMat& src, VkMat& dst, float fov, float cx, float cy, ImInterpolateMode mode)
{
...
double Orthographic2Equidistance_vulkan::filter(const ImMat& src, ImMat& dst, float fov, float cx, float cy, ImInterpolateMode mode)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, fov, cx, cy, mode);
