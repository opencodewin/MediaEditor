#include "fish2sphere_vulkan.h"
#include "fish2sphere_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Fish2Sphere_vulkan::Fish2Sphere_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Fish2Sphere");
    std::vector<vk_specialization_type> specializations(0);
    std::vector<uint32_t> spirv_data;

    if (compile_spirv_module(Fish2Sphere_data, opt, spirv_data) == 0)
    {
...
Fish2Sphere_vulkan::~Fish2Sphere_vulkan()
{
    if (vkdev)
    {
...
void Fish2Sphere_vulkan::upload_param(const VkMat& src, VkMat& dst, float fov, float cx, float cy, ImInterpolateMode mode)
{
...
double Fish2Sphere_vulkan::filter(const ImMat& src, ImMat& dst, float fov, float cx, float cy, ImInterpolateMode mode)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
