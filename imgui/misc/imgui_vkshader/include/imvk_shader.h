...
#define BL_AVERAGE      2 \n\
#define BL_COLOR        3 \n\
#define BL_COLORBURN    4 \n\
#define BL_COLORDODGE   5 \n\
#define BL_DARKEN       6 \n\
#define BL_DIFFERENCE   7 \n\
#define BL_EXCLUSION    8 \n\
#define BL_GLOW         9 \n\
#define BL_HARDLIGHT    10 \n\
#define BL_HARDMIX      11 \n\
#define BL_HUE          12 \n\
#define BL_LIGHTEN      13 \n\
#define BL_LINEARBURN   14 \n\
#define BL_LINEARDODGE  15 \n\
#define BL_LINEARLIGHT  16 \n\
#define BL_LUMINOSITY   17 \n\
#define BL_MULTIPLY     18 \n\
#define BL_NEGATION     19 \n\
#define BL_OVERLAY      20 \n\
#define BL_PHOENIX      21 \n\
#define BL_PINLIGHT     22 \n\
#define BL_REFLECT      23 \n\
#define BL_SATURATION   24 \n\
#define BL_SCREEN       25 \n\
#define BL_SOFTLIGHT    26 \n\
#define BL_SUBTRACT     27 \n\
#define BL_VIVIDLIGHT   28 \n\
\n\
...
#define SHADER_INPUT_RW_DATA \
" \n\
layout (binding = 0) buffer src_int8       { uint8_t   src_data_int8[]; };   \n\
layout (binding = 1) buffer src_int16      { uint16_t  src_data_int16[]; };  \n\
layout (binding = 2) buffer src_float16    { float16_t src_data_float16[]; };\n\
layout (binding = 3) buffer src_float32    { float     src_data_float32[]; };\n\
"

#define SHADER_INPUT_MASK_DATA \
" \n\
...
// Load data for dst as gray
#define SHADER_LOAD_DST_GRAY_INT8 \
" \n\
sfp load_dst_gray_int8(int x, int y, int w, int cstep, int format, float scale) \n\
{ \n\
    sfp rgb_in = sfp(0.f); \n\
    ivec3 i_offset = (y * w + x) * cstep + ivec3(0, 0, 0); \n\
    rgb_in = sfp(uint(dst_data_int8[i_offset.x])) / sfp(scale); \n\
    return rgb_in; \n\
} \n\
"

#define SHADER_LOAD_DST_GRAY_INT16 \
" \n\
sfp load_dst_gray_int16(int x, int y, int w, int cstep, int format, float scale) \n\
{ \n\
    sfp rgb_in = sfp(0.f); \n\
    ivec3 i_offset = (y * w + x) * cstep + ivec3(0, 0, 0); \n\
    rgb_in = sfp(uint(dst_data_int16[i_offset.x])) / sfp(scale); \n\
    return rgb_in; \n\
} \n\
"

#define SHADER_LOAD_DST_GRAY_INT16_BE \
" \n\
sfp load_dst_gray_int16be(int x, int y, int w, int cstep, int format, float scale) \n\
{ \n\
    sfp rgb_in = sfp(0.f); \n\
    ivec3 i_offset = (y * w + x) * cstep + ivec3(0, 0, 0); \n\
    rgb_in = sfp(uint(BE2LE_16BIT(dst_data_int16[i_offset.x]))) / sfp(scale); \n\
    return rgb_in; \n\
} \n\
"

#define SHADER_LOAD_DST_GRAY_FLOAT16 \
" \n\
sfp load_dst_gray_float16(int x, int y, int w, int cstep, int format) \n\
{ \n\
    sfp rgb_in = sfp(0.f); \n\
    ivec3 i_offset = (y * w + x) * cstep + ivec3(0, 0, 0); \n\
    rgb_in = sfp(dst_data_float16[i_offset.x]); \n\
    return rgb_in; \n\
} \n\
"

#define SHADER_LOAD_DST_GRAY_FLOAT32 \
" \n\
sfp load_dst_gray_float32(int x, int y, int w, int cstep, int format) \n\
{ \n\
    sfp rgb_in = sfp(0.f); \n\
    ivec3 i_offset = (y * w + x) * cstep + ivec3(0, 0, 0); \n\
    rgb_in = sfp(dst_data_float32[i_offset.x]); \n\
    return rgb_in; \n\
} \n\
"

#define SHADER_LOAD_DST_GRAY \
SHADER_LOAD_DST_GRAY_INT8 \
SHADER_LOAD_DST_GRAY_INT16 \
SHADER_LOAD_DST_GRAY_INT16_BE \
SHADER_LOAD_DST_GRAY_FLOAT16 \
SHADER_LOAD_DST_GRAY_FLOAT32 \
" \n\
sfp load_dst_gray(int x, int y, int w, int h, int cstep, int format, int type, float scale) \n\
{ \n\
    x = clamp(x, 0, w - 1); \n\
    y = clamp(y, 0, h - 1); \n\
    if (type == DT_INT8) \n\
        return load_dst_gray_int8(x, y, w, cstep, format, scale); \n\
    else if (type == DT_INT16) \n\
        return load_dst_gray_int16(x, y, w, cstep, format, scale); \n\
    else if (type == DT_INT16_BE) \n\
        return load_dst_gray_int16be(x, y, w, cstep, format, scale); \n\
    else if (type == DT_FLOAT16) \n\
        return load_dst_gray_float16(x, y, w, cstep, format); \n\
    else if (type == DT_FLOAT32) \n\
        return load_dst_gray_float32(x, y, w, cstep, format); \n\
    else \n\
        return sfp(0.f); \n\
} \n\
"
...
" \n\
sfp load_gray_int8_"#src"(int x, int y, int w, int cstep, int format) \n\
{ \n\
...
" \n\
sfp load_gray_int16_"#src"(int x, int y, int w, int cstep, int format) \n\
{ \n\
    int i_offset = (y * w + x) * cstep; \n\
    return sfp(uint("#src"_data_int16[i_offset])) / sfp(65535.f); \n\
} \n\
...
" \n\
sfp load_gray_int16be_"#src"(int x, int y, int w, int cstep, int format) \n\
{ \n\
    int i_offset = (y * w + x) * cstep; \n\
    return sfp(uint(BE2LE_16BIT("#src"_data_int16[i_offset]))) / sfp(65535.f); \n\
} \n\
...
" \n\
sfp load_gray_"#src"(int x, int y, int w, int h, int cstep, int format, int type) \n\
{ \n\
    x = clamp(x, 0, w - 1); \n\
    y = clamp(y, 0, h - 1); \n\
    if (type == DT_INT8) \n\
        return load_gray_int8_"#src"(x, y, w, cstep, format); \n\
    else if (type == DT_INT16) \n\
        return load_gray_int16_"#src"(x, y, w, cstep, format); \n\
    else if (type == DT_INT16_BE) \n\
        return load_gray_int16be_"#src"(x, y, w, cstep, format); \n\
    else if (type == DT_FLOAT16) \n\
...
// Load data from dst as rgb
#define SHADER_LOAD_DST_RGB_INT8 \
" \n\
sfpvec3 load_dst_rgb_int8(int x, int y, int w, int cstep, int format) \n\
{ \n\
    sfpvec3 rgb_in = sfpvec3(0.f); \n\
    ivec3 i_offset = (y * w + x) * cstep + color_format_mapping_vec3(format); \n\
    rgb_in.r = sfp(uint(dst_data_int8[i_offset.r])) / sfp(255.f); \n\
...
#define SHADER_LOAD_DST_RGB_INT16 \
" \n\
sfpvec3 load_dst_rgb_int16(int x, int y, int w, int cstep, int format) \n\
{ \n\
    sfpvec3 rgb_in = sfpvec3(0.f); \n\
    ivec3 i_offset = (y * w + x) * cstep + color_format_mapping_vec3(format); \n\
    rgb_in.r = sfp(uint(dst_data_int16[i_offset.r])) / sfp(65535.f); \n\
...
#define SHADER_LOAD_DST_RGB_INT16_BE \
" \n\
sfpvec3 load_dst_rgb_int16be(int x, int y, int w, int cstep, int format) \n\
{ \n\
    sfpvec3 rgb_in = sfpvec3(0.f); \n\
    ivec3 i_offset = (y * w + x) * cstep + color_format_mapping_vec3(format); \n\
    rgb_in.r = sfp(uint(BE2LE_16BIT(dst_data_int16[i_offset.r]))) / sfp(65535.f); \n\
...
#define SHADER_LOAD_DST_RGB_FLOAT16 \
" \n\
sfpvec3 load_dst_rgb_float16(int x, int y, int w, int cstep, int format) \n\
{ \n\
    sfpvec3 rgb_in = sfpvec3(0.f); \n\
    ivec3 i_offset = (y * w + x) * cstep + color_format_mapping_vec3(format); \n\
    rgb_in.r = sfp(dst_data_float16[i_offset.r]); \n\
...
#define SHADER_LOAD_DST_RGB_FLOAT32 \
" \n\
sfpvec3 load_dst_rgb_float32(int x, int y, int w, int cstep, int format) \n\
{ \n\
    sfpvec3 rgb_in = sfpvec3(0.f); \n\
    ivec3 i_offset = (y * w + x) * cstep + color_format_mapping_vec3(format); \n\
    rgb_in.r = sfp(dst_data_float32[i_offset.r]); \n\
...
SHADER_LOAD_DST_RGB_INT8 \
SHADER_LOAD_DST_RGB_INT16 \
SHADER_LOAD_DST_RGB_INT16_BE \
SHADER_LOAD_DST_RGB_FLOAT16 \
SHADER_LOAD_DST_RGB_FLOAT32 \
" \n\
sfpvec3 load_dst_rgb(int x, int y, int w, int h, int cstep, int format, int type) \n\
{ \n\
    x = clamp(x, 0, w - 1); \n\
    y = clamp(y, 0, h - 1); \n\
    if (type == DT_INT8) \n\
        return load_dst_rgb_int8(x, y, w, cstep, format); \n\
    else if (type == DT_INT16) \n\
        return load_dst_rgb_int16(x, y, w, cstep, format); \n\
    else if (type == DT_INT16_BE) \n\
        return load_dst_rgb_int16be(x, y, w, cstep, format); \n\
    else if (type == DT_FLOAT16) \n\
        return load_dst_rgb_float16(x, y, w, cstep, format); \n\
    else if (type == DT_FLOAT32) \n\
        return load_dst_rgb_float32(x, y, w, cstep, format); \n\
    else \n\
        return sfpvec3(0.f); \n\
} \n\
"
...
// Store data as rgba with name
#define SHADER_STORE_RGBA_WITHNAME_INT8(dst) \
" \n\
void store_rgba_int8_"#dst"(sfpvec4 val, int x, int y, int w, int cstep, int format) \n\
{ \n\
    ivec4 o_offset = (y * w + x) * cstep + color_format_mapping_vec4(format); \n\
    "#dst"_data_int8[o_offset.r] = uint8_t(clamp(uint(floor(val.r * sfp(255.0f))), 0, 255)); \n\
    "#dst"_data_int8[o_offset.g] = uint8_t(clamp(uint(floor(val.g * sfp(255.0f))), 0, 255)); \n\
    "#dst"_data_int8[o_offset.b] = uint8_t(clamp(uint(floor(val.b * sfp(255.0f))), 0, 255)); \n\
    "#dst"_data_int8[o_offset.a] = uint8_t(clamp(uint(floor(val.a * sfp(255.0f))), 0, 255)); \n\
} \n\
"

#define SHADER_STORE_RGBA_WITHNAME_INT16(dst) \
" \n\
void store_rgba_int16_"#dst"(sfpvec4 val, int x, int y, int w, int cstep, int format) \n\
{ \n\
    ivec4 o_offset = (y * w + x) * cstep + color_format_mapping_vec4(format); \n\
    "#dst"_data_int16[o_offset.r] = uint16_t(clamp(uint(floor(val.r * sfp(65535.0f))), 0, 65535)); \n\
    "#dst"_data_int16[o_offset.g] = uint16_t(clamp(uint(floor(val.g * sfp(65535.0f))), 0, 65535)); \n\
    "#dst"_data_int16[o_offset.b] = uint16_t(clamp(uint(floor(val.b * sfp(65535.0f))), 0, 65535)); \n\
    "#dst"_data_int16[o_offset.a] = uint16_t(clamp(uint(floor(val.a * sfp(65535.0f))), 0, 65535)); \n\
} \n\
"

#define SHADER_STORE_RGBA_WITHNAME_INT16_BE(dst) \
" \n\
void store_rgba_int16be_"#dst"(sfpvec4 val, int x, int y, int w, int cstep, int format) \n\
{ \n\
    ivec4 o_offset = (y * w + x) * cstep + color_format_mapping_vec4(format); \n\
    "#dst"_data_int16[o_offset.r] = BE2LE_16BIT(uint16_t(clamp(uint(floor(val.r * sfp(65535.0f))), 0, 65535))); \n\
    "#dst"_data_int16[o_offset.g] = BE2LE_16BIT(uint16_t(clamp(uint(floor(val.g * sfp(65535.0f))), 0, 65535))); \n\
    "#dst"_data_int16[o_offset.b] = BE2LE_16BIT(uint16_t(clamp(uint(floor(val.b * sfp(65535.0f))), 0, 65535))); \n\
    "#dst"_data_int16[o_offset.a] = BE2LE_16BIT(uint16_t(clamp(uint(floor(val.a * sfp(65535.0f))), 0, 65535))); \n\
} \n\
"

#define SHADER_STORE_RGBA_WITHNAME_FLOAT16(dst) \
" \n\
void store_rgba_float16_"#dst"(sfpvec4 val, int x, int y, int w, int cstep, int format) \n\
{ \n\
    ivec4 o_offset = (y * w + x) * cstep + color_format_mapping_vec4(format); \n\
    "#dst"_data_float16[o_offset.r] = float16_t(clamp(val.r, sfp(0.f), sfp(1.f))); \n\
    "#dst"_data_float16[o_offset.g] = float16_t(clamp(val.g, sfp(0.f), sfp(1.f))); \n\
    "#dst"_data_float16[o_offset.b] = float16_t(clamp(val.b, sfp(0.f), sfp(1.f))); \n\
    "#dst"_data_float16[o_offset.a] = float16_t(clamp(val.a, sfp(0.f), sfp(1.f))); \n\
} \n\
"

#define SHADER_STORE_RGBA_WITHNAME_FLOAT32(dst) \
" \n\
void store_rgba_float32_"#dst"(sfpvec4 val, int x, int y, int w, int cstep, int format) \n\
{ \n\
    ivec4 o_offset = (y * w + x) * cstep + color_format_mapping_vec4(format); \n\
    "#dst"_data_float32[o_offset.r] = float(clamp(val.r, sfp(0.f), sfp(1.f))); \n\
    "#dst"_data_float32[o_offset.g] = float(clamp(val.g, sfp(0.f), sfp(1.f))); \n\
    "#dst"_data_float32[o_offset.b] = float(clamp(val.b, sfp(0.f), sfp(1.f))); \n\
    "#dst"_data_float32[o_offset.a] = float(clamp(val.a, sfp(0.f), sfp(1.f))); \n\
} \n\
"

#define SHADER_STORE_WITHNAME_RGBA(dst) \
SHADER_STORE_RGBA_WITHNAME_INT8(dst) \
SHADER_STORE_RGBA_WITHNAME_INT16(dst) \
SHADER_STORE_RGBA_WITHNAME_INT16_BE(dst) \
SHADER_STORE_RGBA_WITHNAME_FLOAT16(dst) \
SHADER_STORE_RGBA_WITHNAME_FLOAT32(dst) \
" \n\
void store_rgba_"#dst"(sfpvec4 val, int x, int y, int w, int h, int cstep, int format, int type) \n\
{ \n\
    x = clamp(x, 0, w - 1); \n\
    y = clamp(y, 0, h - 1); \n\
    if (type == DT_INT8) \n\
        store_rgba_int8_"#dst"(val, x, y, w, cstep, format); \n\
    else if (type == DT_INT16) \n\
        store_rgba_int16_"#dst"(val, x, y, w, cstep, format); \n\
    else if (type == DT_INT16_BE) \n\
        store_rgba_int16be_"#dst"(val, x, y, w, cstep, format); \n\
    else if (type == DT_FLOAT16) \n\
        store_rgba_float16_"#dst"(val, x, y, w, cstep, format); \n\
    else if (type == DT_FLOAT32) \n\
        store_rgba_float32_"#dst"(val, x, y, w, cstep, format); \n\
} \n\
"
...
#define SHADER_LOAD_DST_IMAGE \
SHADER_LOAD_DST_GRAY \
SHADER_LOAD_DST_RGB \
SHADER_LOAD_DST_RGBA \
" \n\
sfpvec4 load_dst_rgb_image(int x, int y, int w, int h, int cstep, int format, int type) \n\
{ \n\
    sfpvec4 rgba_result = sfpvec4(0.f); \n\
    if (format == CF_ABGR || format == CF_ARGB || format == CF_BGRA || format == CF_RGBA) \n\
    { \n\
        rgba_result = load_dst_rgba(x, y, w, h, cstep, format, type); \n\
    } \n\
    else if (format == CF_BGR || format == CF_RGB) \n\
    { \n\
        sfpvec3 rgb = load_dst_rgb(x, y, w, h, cstep, format, type); \n\
        rgba_result = sfpvec4(rgb, sfp(1.0)); \n\
    } \n\
    else if (format == CF_GRAY) \n\
    { \n\
        sfp scale = type == DT_INT8 ? sfp(255) : type == DT_INT16 ? sfp(65535) : sfp(1.0); \n\
        sfp gray = load_dst_gray(x, y, w, h, cstep, format, type, scale); \n\
        rgba_result = sfpvec4(gray, gray, gray, sfp(1.0)); \n\
    } \n\
    return rgba_result; \n\
} \
...
#define SHADER_STORE_IMAGE \
SHADER_STORE_GRAY \
SHADER_STORE_RGB \
SHADER_STORE_RGBA \
" \n\
void store_rgb_image(sfpvec4 val, int x, int y, int w, int h, int cstep, int format, int type) \n\
{ \n\
    if (format == CF_ABGR || format == CF_ARGB || format == CF_BGRA || format == CF_RGBA) \n\
    { \n\
        store_rgba(val, x, y, w, h, cstep, format, type); \n\
    } \n\
    else if (format == CF_BGR || format == CF_RGB) \n\
    { \n\
        store_rgb(val.rgb, x, y, w, h, cstep, format, type); \n\
    } \n\
    else if (format == CF_GRAY) \n\
    { \n\
        store_gray(val.x, x, y, w, h, cstep, format, type); \n\
    } \n\
} \
"

// resize for single channel(gray)
#define GRAY_LOAD \
" \n\
sfp load_gray(int x, int y) \n\
{ \n\
    return load_gray(x, y, p.w, p.h, 1, p.in_format, p.in_type, p.in_scale); \n\
} \
"

#define GRAY_INTERPLATE_NONE \
" \n\
sfp gray_interplate_none(int x, int y) \n\
{ \n\
    return load_gray(x, y); \n\
} \
"

#define GRAY_INTERPLATE_NEAREST \
" \n\
sfp gray_interplate_nearest(int x, int y) \n\
{ \n\
    ...
    srcx = min(srcx, p.w - 1); \n\
    srcy = min(srcy, p.h - 1); \n\
    return load_gray(srcx, srcy); \n\
} \
"

#define GRAY_INTERPLATE_BILINEAR \
" \n\
sfp gray_interplate_bilinear(int x, int y) \n\
{ \n\
    float fx = float(p.out_w) / float(p.w); \n\
    float fy = float(p.out_h) / float(p.h); \n\
    float srcx = x / fx; \n\
    ...
    sfp _v = sfp(0.f); \n\
    sfp _x_y   = load_gray(_x,     _y    ); \n\
    sfp _x1_y  = load_gray(_x + 1, _y    ); \n\
    sfp _x_y1  = load_gray(_x,     _y + 1); \n\
    sfp _x1_y1 = load_gray(_x + 1, _y + 1); \n\
...
#define GRAY_INTERPLATE_BICUBIC \
" \n\
sfp gray_interplate_bicubic(int x, int y) \n\
{ \n\
    ...
    \n\
    sfp _v = sfp(0.f); \n\
    sfp _x_1_y_1 = load_gray(sx-1, sy-1); \n\
    sfp _x_1_y   = load_gray(sx-1, sy  ); \n\
    sfp _x_1_y1  = load_gray(sx-1, sy+1); \n\
    sfp _x_1_y2  = load_gray(sx-1, sy+2); \n\
    sfp _x_y_1   = load_gray(sx,   sy-1); \n\
    sfp _x_y     = load_gray(sx,   sy  ); \n\
    sfp _x_y1    = load_gray(sx,   sy+1); \n\
    sfp _x_y2    = load_gray(sx,   sy+2); \n\
    sfp _x1_y_1  = load_gray(sx+1, sy-1); \n\
    sfp _x1_y    = load_gray(sx+1, sy  ); \n\
    sfp _x1_y1   = load_gray(sx+1, sy+1); \n\
    sfp _x1_y2   = load_gray(sx+1, sy+2); \n\
    sfp _x2_y_1  = load_gray(sx+2, sy-1); \n\
    sfp _x2_y    = load_gray(sx+2, sy  ); \n\
    sfp _x2_y1   = load_gray(sx+2, sy+1); \n\
    sfp _x2_y2   = load_gray(sx+2, sy+2); \n\
    \n\
    _v = \n\
...
#define GRAY_INTERPLATE_AREA \
" \n\
sfp gray_interplate_area(int x, int y) \n\
{ \n\
    sfp _v = sfp(0.f); \n\
    sfp scale_x = sfp(p.w) / sfp(p.out_w); \n\
    sfp scale_y = sfp(p.h) / sfp(p.out_h); \n\
    ...
    { \n\
        ...
        _v = load_gray(sx,     sy    ) * cbufx[0] * cbufy[0] + \n\
             load_gray(sx,     sy + 1) * cbufx[0] * cbufy[1] + \n\
             load_gray(sx + 1, sy    ) * cbufx[1] * cbufy[0] + \n\
             load_gray(sx + 1, sy + 1) * cbufx[1] * cbufy[1]; \n\
    } \n\
    return _v; \n\
} \
"

#define GRAY_INTERPLATE \
GRAY_LOAD \
GRAY_INTERPLATE_NONE \
GRAY_INTERPLATE_NEAREST \
GRAY_INTERPLATE_BILINEAR \
GRAY_INTERPLATE_BICUBIC \
GRAY_INTERPLATE_AREA \
" \n\
sfp gray_interplate(int x, int y) \n\
{ \n\
    if (p.interp_type == INTERPOLATE_NONE) \n\
        return gray_interplate_none(x, y); \n\
    if (p.interp_type == INTERPOLATE_NEAREST) \n\
        return gray_interplate_nearest(x, y); \n\
    else if (p.interp_type == INTERPOLATE_BILINEAR) \n\
        return gray_interplate_bilinear(x, y); \n\
    else if (p.interp_type == INTERPOLATE_BICUBIC) \n\
        return gray_interplate_bicubic(x, y); \n\
    else if (p.interp_type == INTERPOLATE_AREA) \n\
        return gray_interplate_area(x, y); \n\
    else \n\
        return gray_interplate_nearest(x, y); \n\
} \
"
