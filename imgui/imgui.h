...
#include <math.h>                   // add by Dicky for fabs
...
namespace ImGui
{
    ...
    IMGUI_API void          PushStyleVar(ImGuiStyleVar idx, bool val);                     // modify a style bool variable. always use this if you modify the style after NewFrame().
    // add by Dicky end
    ...
    // add by Dicky
    IMGUI_API void          SeparatorV();  // vertical separator
    // add by Dicky end
    ...
    IMGUI_API ImU32         ColorBrightnessU32(const ImU32& in, float bright); // for change color brightness
    // add by Dicky end
    ...
    IMGUI_API void          SetNextFrameWantCaptureMouse(bool want_capture_mouse = true);       // modify by Dicky add default, Override io.WantCaptureMouse flag next frame (said flag is left for your application to handle, typical when true it instucts your app to ignore inputs). This is equivalent to setting "io.WantCaptureMouse = want_capture_mouse;" after the next NewFrame() call.
...
enum ImGuiWindowFlags_
{
    ...
    // add By Dicky for bold/shadow title label for window if window has title bar
    ImGuiWindowFlags_TitleBarBold           = 1 << 22, // add shadow for title bar label if enabled
    ImGuiWindowFlags_FullScreen             = 1 << 23,  // Create with Full Screen effect on viewport
    // add By Dicky end
...
enum ImGuiStyleVar_
{
    ...
    ImGuiStyleVar_DoFrameShadingForMultilineText, // bool DoFrameShadingForMultilineText
    ImGuiStrleVar_ButtonBorderSize,         // float     ButtonBorderSize
    ImGuiStrleVar_DoButtonBorder,           // bool      DoButtonBorder
	// add By Dicky end
...
struct ImFontAtlas
{
    ...
    // add By Dicky for scale font
    IMGUI_API ImFont*           AddFontDefault(const ImFontConfig* font_cfg_template, float scale);
    IMGUI_API ImFont*           AddFontDefault(float scale);
    // add By Dicky end
    ...
    // Add By Dicky
    IMGUI_API const ImWchar*    GetGlyphRangesAscII();                  // Default + Half-Width
    IMGUI_API const ImWchar*    GetGlyphRangesChineseOnly();            // Japanese Hiragana/Katakana + full set of about 21000 CJK Unified Ideographs
    // Add By Dicky end
...
enum ImGuiViewportFlags_
{
    ...
    // Add By Dicky
    ImGuiViewportFlags_FullScreen               = 1 << 14   // Platform Window is full screen
    // Add By Dicky end
};
