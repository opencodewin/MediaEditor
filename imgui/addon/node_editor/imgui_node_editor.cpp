...
void ed::Node::GetGroupedNodes(std::vector<Node*>& result, bool append, ImVec2 expand)
{
    ...
    Editor->FindNodesInRect(rect, _nodes, false, false);
...
void ed::Link::Draw(ImDrawList* drawList, DrawFlags flags)
{
    ...
    else if (flags & Selected)
    {
        ...
        Draw(drawList, borderColor, 6.0f);
    }
...
void ed::EditorContext::Begin(const char* id, const ImVec2& size)
{
    ...
    if (m_DragOverBorder && m_CurrentAction && m_CurrentAction->IsDragging() && m_NavigateAction.MoveOverEdge(canvasSize))
    {
        ...
        for (int i = 0; i < IM_ARRAYSIZE(ImGuiIO::MouseClickedPos); ++i)
...
void ed::EditorContext::End()
{
    ...
    if (m_NavigateAction.m_IsActive)
    {
        ...
        m_CurrentAction = &m_NavigateAction;
    }
    else
...
{
    m_CreateItemAction.DrawLastLine(light);
}
...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control& control)
{
    ...
    // #debug
    // draw mouse pos
    //Editor->GetDrawList()->AddCircleFilled(io.MousePos, 4.0f, IM_COL32(255, 0, 255, 255));
...
bool ed::NavigateAction::HandleZoom(const Control& control)
{
    ...
    auto& io = ImGui::GetIO();
    const bool is_shift_key_only = io.KeyShift && !io.KeyCtrl && !io.KeyAlt && !io.KeySuper; //(io.KeyMods & ImGuiMod_Shift);
...
bool ed::SizeAction::Process(const Control& control)
{
    ...
    if (m_SizedNode && control.ActiveNode == m_SizedNode)
    {
...
void ed::CreateItemAction::DrawLastLine(bool light)
{
    ...
    ed::Link candidate(Editor, 0);
    ImU32 color = light ? IM_COL32_BLACK : IM_COL32_WHITE;
    candidate.m_Color = color;
    candidate.m_StartPin = &startPin;
    candidate.m_EndPin = &endPin;
    auto drawList = ImGui::GetWindowDrawList();
    drawList->ChannelsSetCurrent(c_LinkChannel_NewLink);
    candidate.UpdateEndpoints();
    candidate.Draw(drawList, color , m_LinkThickness);
}
