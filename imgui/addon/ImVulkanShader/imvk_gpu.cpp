...
static GpuInfo* g_gpu_infos[MAX_GPU_COUNT] = {0};
...
int create_gpu_instance()
{
    ...
    // fprintf(stderr, "create_gpu_instance");
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkEnumerateInstanceLayerProperties failed %d", ret);
        return -1;
    }
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkEnumerateInstanceExtensionProperties failed %d", ret);
        return -1;
    }
    ...
    if (vkEnumerateInstanceVersion)
    {
        ...
        if (ret != VK_SUCCESS)
        {
            fprintf(stderr, "vkEnumerateInstanceVersion failed %d", ret);
            return -1;
        }
    }
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkCreateInstance failed %d", ret);
        return -1;
    }
    ...
    if (support_VK_EXT_debug_utils)
    {
        ...
        if (ret != VK_SUCCESS)
        {
            fprintf(stderr, "CreateDebugUtilsMessengerEXT failed %d", ret);
            return -1;
        }
    }
#endif // ENABLE_VALIDATION_LAYER
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkEnumeratePhysicalDevices failed %d", ret);
        return -1;
    }
    ...
    for (uint32_t i = 0; i < physicalDeviceCount; i++)
    {
        ...
        if (ret != VK_SUCCESS)
        {
            fprintf(stderr, "vkEnumerateDeviceExtensionProperties failed %d", ret);
            return -1;
        }
        ...
        fprintf(stderr, "[%u %s]  bugsbn1=%d  bugbilz=%d  bugcopc=%d  bugihfa=%d", i, physicalDeviceProperties.deviceName,
        ...
        fprintf(stderr, "[%u %s]  fp16-p/s/u/a=%d/%d/%d/%d  int8-p/s/u/a=%d/%d/%d/%d", i, physicalDeviceProperties.deviceName,
        ...
        fprintf(stderr, "[%u %s]  subgroup=%u  basic/vote/ballot/shuffle=%d/%d/%d/%d", i, physicalDeviceProperties.deviceName,
    ...
    // atexit() seems to be helpful for calling it earlier
...
public:
    ...
    // utility operator
    const Packing_vulkan* get_utility_operator(int storage_type_from, int storage_type_to, int cast_type_from_index, int cast_type_to_index, int packing_type_to_index) const;
    ...
    mutable Packing_vulkan* uop_packing[2][2][3][3][3];
...
const Packing_vulkan* VulkanDevicePrivate::get_utility_operator(int storage_type_from, int storage_type_to, int cast_type_from_index, int cast_type_to_index, int packing_type_to_index) const
{
    MutexLockGuard lock(uop_lock);

    const Packing_vulkan* cached_uop = uop_packing[storage_type_from][storage_type_to][cast_type_from_index][cast_type_to_index][packing_type_to_index];
    if (cached_uop)
        return cached_uop;

    if ((cast_type_from_index == 1 && cast_type_to_index == 2) || (cast_type_from_index == 2 && cast_type_to_index == 1))
    {
        fprintf(stderr, "no fp16p to/from fp16s conversion");
        return 0;
    }
    ...
    {
        fprintf(stderr, "cannot create uop with use_fp16_packed if not support_fp16_packed");
        return 0;
    }

    if (!vkdev->info.support_fp16_storage() && opt.use_fp16_storage)
    {
        fprintf(stderr, "cannot create uop with use_fp16_storage if not support_fp16_storage");
        return 0;
    }
            ...
            for (int j0 = 0; j0 < 3; j0++)
            {
                for (int j1 = 0; j1 < 3; j1++)
                {
                    ...
                    for (int k = 0; k < 3; k++)
                    {
                        ...
                        Packing_vulkan* uop = uop_packing[i0][i1][j0][j1][k];
                        if (!uop)
                            continue;
    ...
    : info(get_gpu_info(device_index)), d(new VulkanDevicePrivate(this)), device_number(device_index)
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkCreateDevice failed %d", ret);
    }
    ...
    // prepare immutable texelfetch sampler
    {
        ...
        if (ret != VK_SUCCESS)
        {
            fprintf(stderr, "vkCreateSampler failed %d", ret);
        }
    }
...
VkShaderModule VulkanDevice::compile_shader_module(const uint32_t* spv_data, size_t spv_data_size) const
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkCreateShaderModule failed %d", ret);
        return 0;
    }
...
int VulkanDevice::create_descriptorset_layout(int binding_count, const int* binding_types, VkDescriptorSetLayout* descriptorset_layout) const
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkCreateDescriptorSetLayout failed %d", ret);
        return -1;
    }
...
int VulkanDevice::create_pipeline_layout(int push_constant_count, VkDescriptorSetLayout descriptorset_layout, VkPipelineLayout* pipeline_layout) const
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkCreatePipelineLayout failed %d", ret);
        return -1;
    }
...
int VulkanDevice::create_pipeline(VkShaderModule shader_module, VkPipelineLayout pipeline_layout, const std::vector<vk_specialization_type>& specializations, VkPipeline* pipeline) const
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkCreateComputePipelines failed %d", ret);
        return -1;
    }
...
int VulkanDevice::create_descriptor_update_template(int binding_count, const int* binding_types, VkDescriptorSetLayout descriptorset_layout, VkPipelineLayout pipeline_layout, VkDescriptorUpdateTemplateKHR* descriptor_update_template) const
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkCreateDescriptorUpdateTemplateKHR failed %d", ret);
        return -1;
    }
...
uint32_t VulkanDevice::find_memory_index(uint32_t memory_type_bits, VkFlags required, VkFlags preferred, VkFlags preferred_not) const
{
    ...
    fprintf(stderr, "no such memory type %u %u %u %u", memory_type_bits, required, preferred, preferred_not);
    return -1;
}
...
VkQueue VulkanDevice::acquire_queue(uint32_t queue_family_index) const
{
    ...
    {
        fprintf(stderr, "invalid queue_family_index %u", queue_family_index);
        return 0;
    }
    ...
    if (!queue)
    {
        fprintf(stderr, "FATAL ERROR! out of hardware queue %u\n", queue_family_index);
    }
...
void VulkanDevice::reclaim_blob_allocator(VkAllocator* allocator) const
{
    ...
    fprintf(stderr, "FATAL ERROR! reclaim_blob_allocator get wild allocator %p", allocator);
}
...
void VulkanDevice::reclaim_staging_allocator(VkAllocator* allocator) const
{
    ...
    fprintf(stderr, "FATAL ERROR! reclaim_staging_allocator get wild allocator %p", allocator);
}
...
static void ParserLog(const char* prog, const char *log)
{
    ...
    if (!message.empty())
    {
        ...
        for (int i = 0; i < (int)logs.size() - 2; i++)
        {
...
int compile_spirv_module(const char* comp_data, int comp_data_size, const Option& opt, std::vector<uint32_t>& spirv, std::string& log)
{
    ...
    if (opt.use_fp16_storage)
    {
        ...
        custom_defines.push_back(std::make_pair("eps", "float16_t(1e-4)"));
        custom_defines.push_back(std::make_pair("sfpvec2", "f16vec2"));
        custom_defines.push_back(std::make_pair("sfpvec3", "f16vec3"));
    ...
    else if (opt.use_fp16_packed)
    {
        ...
        custom_defines.push_back(std::make_pair("eps", "float(1e-8)"));
    ...
    if (opt.use_fp16_storage)
    {
        custom_defines.push_back(std::make_pair("ImVulkan_fp16_storage", "1"));
    }
    else if (opt.use_fp16_packed)
    {
        custom_defines.push_back(std::make_pair("ImVulkan_fp16_packed", "1"));
    }

    if (opt.use_fp16_uniform)
    {
        custom_defines.push_back(std::make_pair("ImVulkan_fp16_uniform", "1"));
    }

    if (opt.use_fp16_arithmetic)
    {
        custom_defines.push_back(std::make_pair("ImVulkan_fp16_arithmetic", "1"));
    }

    if (opt.use_int8_storage)
    {
        custom_defines.push_back(std::make_pair("ImVulkan_int8_storage", "1"));
    }
    else if (opt.use_int8_packed)
    {
        custom_defines.push_back(std::make_pair("ImVulkan_int8_packed", "1"));
    }

    if (opt.use_int8_uniform)
    {
        custom_defines.push_back(std::make_pair("ImVulkan_int8_uniform", "1"));
    }

    if (opt.use_int8_arithmetic)
    {
        custom_defines.push_back(std::make_pair("ImVulkan_int8_arithmetic", "1"));
    }

    if (opt.use_image_storage)
    {
        custom_defines.push_back(std::make_pair("ImVulkan_image_shader", "1"));
    }

    if (opt.use_subgroup_basic)
    {
        custom_defines.push_back(std::make_pair("ImVulkan_subgroup_basic", "1"));

        if (opt.use_subgroup_vote)
        {
            custom_defines.push_back(std::make_pair("ImVulkan_subgroup_vote", "1"));
        }
        if (opt.use_subgroup_ballot)
        {
            custom_defines.push_back(std::make_pair("ImVulkan_subgroup_ballot", "1"));
        }
...

    {
        ...
        else
        {
            ...
            else
            {
                ...
                auto* ir = program.getIntermediate(EShLangCompute);
