                                ...
                                ShaderInfo& shader_info) const
{
    ...
    if (ret != 0)
    {
        fprintf(stderr, "resolve_shader_info failed %d", ret);
        return -1;
    }
    ...
    if (!shader_module)
    {
        fprintf(stderr, "create_shader_module failed");
        return -1;
    }
    ...
    if (ret != 0)
    {
        fprintf(stderr, "new_pipeline failed");
                                ...
                                VkDescriptorUpdateTemplateKHR* _descriptor_update_template) const
{
    ...
    {
        fprintf(stderr, "pipeline specialization count mismatch, expect %d but got %d", shader_info.specialization_count, (int)specializations.size());
