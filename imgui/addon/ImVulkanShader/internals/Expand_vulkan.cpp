...
#include "Expand_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Expand_vulkan::Expand_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Expand");

    std::vector<vk_specialization_type> specializations(0);

    if (compile_spirv_module(ExpandShader_data, opt, spirv_data) == 0)
    {
...
void Expand_vulkan::upload_param(const VkMat& src, VkMat& dst, int t, int b, int l, int r) const
{
...
double Expand_vulkan::expand(const ImMat& src, ImMat& dst, int t, int b, int l, int r) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    VkMat dst_gpu;
    dst_gpu.create_type(src.w + l + r, src.h + t + b, 4, dst.type, opt.blob_vkallocator);
    ...
    upload_param(src_gpu, dst_gpu, t, b, l, r);
