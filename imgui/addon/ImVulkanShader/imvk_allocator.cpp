...
int VkAllocator::flush(VkBufferMemory* ptr)
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkFlushMappedMemoryRanges failed %d", ret);
        return -1;
    }
...
int VkAllocator::invalidate(VkBufferMemory* ptr)
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkInvalidateMappedMemoryRanges failed %d", ret);
        return -1;
    }
...
VkBuffer VkAllocator::create_buffer(size_t size, VkBufferUsageFlags usage)
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkCreateBuffer failed %d", ret);
        return 0;
    }
...
VkDeviceMemory VkAllocator::allocate_memory(size_t size, uint32_t memory_type_index)
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkAllocateMemory failed %d", ret);
        return 0;
    }
...
VkImage VkAllocator::create_image(int width, int height, int depth, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage)
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkCreateImage failed %d %d %d %d %d %d %d", ret, width, height, depth, format, tiling, usage);
        return 0;
    }
...
VkImageView VkAllocator::create_imageview(VkImage image, VkFormat format)
{
    ...
    if (ret != VK_SUCCESS)
    {
        fprintf(stderr, "vkCreateImageView failed %d", ret);
        return 0;
    }
...
void VkBlobAllocator::fastFree(VkBufferMemory* ptr)
{
    ...
    if (block_index == -1)
    {
        fprintf(stderr, "FATAL ERROR! unlocked VkBlobAllocator get wild %p", ptr->buffer);
...
VkImageMemory* VkBlobAllocator::fastMalloc(int w, int h, int c, size_t elemsize, int elempack)
{
    ...
    {
        fprintf(stderr, "image dimension too large %d %d %d > %d", width, height, depth, (int)vkdev->info.max_image_dimension_3d());
        return 0;
    }
...
{
    //     fprintf(stderr, "VkWeightAllocator %lu %lu", d->buffer_blocks.size(), d->dedicated_buffer_blocks.size());
...
void VkStagingAllocator::set_size_compare_ratio(float scr)
{
    if (scr < 0.f || scr > 1.f)
    {
        fprintf(stderr, "invalid size compare ratio %f", scr);
        return;
    }
