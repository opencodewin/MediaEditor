...
#include "Brightness_shader.h"
#include "ImVulkanShader.h"

namespace ImGui 
{
Brightness_vulkan::Brightness_vulkan(int gpu)
{
...
#endif
    cmd = new VkCompute(vkdev, "Brightness");
...
Brightness_vulkan::~Brightness_vulkan()
{
    if (vkdev)
    {
...
void Brightness_vulkan::upload_param(const VkMat& src, VkMat& dst, float brightness) const
{
    ...
    constants[10].f = brightness;
    cmd->record_pipeline(pipe, bindings, constants, dst);
}

double Brightness_vulkan::filter(const ImMat& src, ImMat& dst, float brightness) const
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, brightness);
