...
#endif
    cmd = new VkCompute(vkdev, "Bilateral");
...
void Bilateral_vulkan::upload_param(const VkMat& src, VkMat& dst, int ksz, float sigma_spatial, float sigma_color)
{
    ...
    else if (dst.type == IM_DT_INT16 || dst.type == IM_DT_INT16_BE || dst.type == IM_DT_INT16_BE)    bindings[1] = dst;
    else if (dst.type == IM_DT_FLOAT16)  bindings[2] = dst;
    else if (dst.type == IM_DT_FLOAT32)  bindings[3] = dst;

    if      (src.type == IM_DT_INT8)     bindings[4] = src;
    else if (src.type == IM_DT_INT16 || src.type == IM_DT_INT16_BE || src.type == IM_DT_INT16_BE)    bindings[5] = src;
...
double Bilateral_vulkan::filter(const ImMat& src, ImMat& dst, int ksz, float sigma_spatial, float sigma_color)
{
    double ret = 0.0;
    if (!vkdev || !pipe || !cmd)
    {
    ...
    upload_param(src_gpu, dst_gpu, ksz, sigma_spatial, sigma_color);
