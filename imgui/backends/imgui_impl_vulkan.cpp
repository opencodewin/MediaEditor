...
{
    ImGui_ImplVulkan_InitInfo   *VulkanInitInfo; // modify by Dicky
    ...
    //VkDescriptorSet             FontDescriptorSet; // Disable by Dicky
    ImTextureVk                 FontTexture;  // add By Dicky
    ...
    // Add By Dicky
    ~ImGui_ImplVulkan_Data() { if (FontTexture) delete FontTexture; }
    // Add By Dicky end
};
...
bool ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    if (bd->FontView || bd->FontImage || bd->FontMemory || (bd->FontTexture && bd->FontTexture->textureDescriptor)) // modify by Dicky
    {
    ...
    font_texture->textureImageMemory = bd->FontMemory;
    font_texture->textureDescriptor = ImGui_ImplVulkan_AddTexture(bd->FontSampler, bd->FontView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    // modify By Dicky
...
void ImGui_ImplVulkan_DestroyFontsTexture()
{
	...
	// modify by Dicky
    if (bd->FontTexture && bd->FontTexture->textureDescriptor)
    {
        ImGui_ImplVulkan_RemoveTexture(bd->FontTexture->textureDescriptor);
        bd->FontTexture->textureDescriptor = VK_NULL_HANDLE;
        io.Fonts->SetTexID(0);
    }
    // modify by Dicky end
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info)
{
    ...
    bd->VulkanInitInfo = info; // modify by Dicky
...
void ImGui_ImplVulkan_NewFrame()
{
    ...
    if (!bd->FontTexture || !bd->FontTexture->textureDescriptor) // modify by Dicky
...
void ImGui_ImplVulkan_SetMinImageCount(uint32_t min_image_count)
{
    ...
    if (bd->VulkanInitInfo->MinImageCount == min_image_count)  // modify by Dicky
        return;
    ...
    bd->VulkanInitInfo->MinImageCount = min_image_count; // modify by Dicky
}
...
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (!bd) return {}; // Add By Dicky
    ImGui_ImplVulkan_InitInfo* v = bd->VulkanInitInfo;
    if (!v || !v->PhysicalDevice) return {}; // Add By Dicky
...
static void createBuffer(ImGui_ImplVulkan_InitInfo* v, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
    ...
    allocInfo.memoryTypeIndex = findMemoryType(v, memRequirements.memoryTypeBits, properties);
...
static void createImage(ImGui_ImplVulkan_InitInfo* v, uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) 
{
    ...
    if (vkCreateImage(v->Device, &imageInfo, nullptr, &image) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to create image!");
    }
    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(v->Device, image, &memRequirements);
    VkMemoryAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocInfo.allocationSize = memRequirements.size;
    allocInfo.memoryTypeIndex = findMemoryType(v, memRequirements.memoryTypeBits, properties);
    if (vkAllocateMemory(v->Device, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) 
    {
...
static void createImageView(ImGui_ImplVulkan_InitInfo* v, VkImage image, VkImageView& imageView)
{
    ...
    info.subresourceRange.levelCount = 1;
    info.subresourceRange.layerCount = 1;
    if (vkCreateImageView(v->Device, &info, nullptr, &imageView) != VK_SUCCESS)
    {
...
static void createImageSampler(ImGui_ImplVulkan_InitInfo* v, VkSampler& textureSampler)
{
    ...
    info.maxLod = 1000;
    info.maxAnisotropy = 1.0f;
    if (vkCreateSampler(v->Device, &info, nullptr, &textureSampler) != VK_SUCCESS)
    {
...
ImTextureID ImGui_ImplVulkan_CreateTexture(const void * pixels, int width, int height, int channels, int bit_depth)
{
    ...
    else if (bit_depth == 32)
    {
        ...
        createImage(v, width, height, VK_FORMAT_R32G32B32A32_SFLOAT,
                    VK_IMAGE_TILING_OPTIMAL,
                    VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
