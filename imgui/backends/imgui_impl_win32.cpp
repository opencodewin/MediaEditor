...
static bool ImGui_ImplWin32_InitEx(void* hwnd, bool platform_has_own_dc)
{
    ...
    bd->LastMouseCursor = ImGuiMouseCursor_Backend; // modify by Dicky
...
#endif // Modify by Dicky to disable for MinGW, maybe we enable later after we modify cmake file
//---------------------------------------------------------------------------------------------------------
...
void ImGui_ImplWin32_WaitForEvent()
{
    ...
    if (waiting_time > 0.0)
    {
        if (!isinf(waiting_time) && waiting_time <= 2.0)
        {
            ...
            if (ImGui::GetIO().ConfigFlags & ImGuiConfigFlags_EnablePowerSavingMode)
                ::MsgWaitForMultipleObjectsEx(0, nullptr, waiting_time_ms, QS_ALLINPUT, MWMO_INPUTAVAILABLE|MWMO_ALERTABLE);
            else
...
void ImGui_ImplWin32_FullScreen(ImGuiViewport* viewport, bool on)
{
    if ((ImGui::GetIO().ConfigFlags & ImGuiConfigFlags_ViewportsEnable))
    {
        ...
        ::SetWindowPos(vd->Hwnd, on ? HWND_TOPMOST : nullptr, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, on ? SWP_SHOWWINDOW : SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
    }
    else
    {
        ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
        ...
        ::SetWindowPos(bd->hWnd, on ? HWND_TOPMOST : nullptr, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, on ? SWP_SHOWWINDOW : SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
    }
}
// Add By Dicky end
